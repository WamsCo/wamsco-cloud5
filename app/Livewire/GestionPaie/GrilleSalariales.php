    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_pointe_vente == 1){
                ...
                $grille = GrilleSalariale :: where('societe_id',auth()->user()->societe_id)->where('categorie','like','%'.$this->query.'%')->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage); 
    ...
    public function store(){        
        ...
        if($test > 0){
            ...
            if($autoriser == 1){ 
                ...
                $grilleSal = GrilleSalariale :: create(['reference'=>$reference,'categorie'=>$this->categorie,'echelon'=>$this->echelon,'salaire_base'=>$this->salaire_base,'societe'=>auth()->user()->societe,
                'societe_id'=>auth()->user()->societe_id,'nom_user'=>auth()->user()->email,'user_id'=>auth()->user()->id]);
                // ceci recupere le dernier enregistrement cree a l'instant
                $dernier_id = $grilleSal->id; 
    ...
    public function update(){
        ...
        if($test > 0){
            ...
            if($autoriser == 1){  
                if($this->ids){
                    $reference = 'CAT-'.$this->categorie.''.$this->echelon;
                    GrilleSalariale::find($this->ids)->update(['reference'=>$reference,'categorie'=>$this->categorie,'echelon'=>$this->echelon,'salaire_base'=>$this->salaire_base,'societe'=>auth()->user()->societe,
                    'societe_id'=>auth()->user()->societe_id,'nom_user'=>auth()->user()->email,'user_id'=>auth()->user()->id]);
