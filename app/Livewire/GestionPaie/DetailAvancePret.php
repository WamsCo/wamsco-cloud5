...
class DetailAvancePret extends Component
{
    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){ 
            if($mod_ticket == 1){             
                ...
                $id = request('id'); // id 
                $test_avance = AvancePret::where('societe',auth()->user()->societe)->where('id',$id)->count();
                if($test_avance > 0){
                    
                    $avances = AvancePret::where('societe',auth()->user()->societe)->where('id',$id)->first();                           
                    $this->ids = $avances->id;
    ...
    public function precedant(int $id){ 
        $testPrecedant = AvancePret::where('societe',auth()->user()->societe)->where('id','<',$id)->orderBy('id','desc')->count();
        if($testPrecedant > 0){ 
            $precedant = AvancePret::where('societe',auth()->user()->societe)->where('id','<',$id)->orderBy('id','desc')->first();        
    ...
    public function suivant(int $id){    
        ...
        $testSuivant = AvancePret::where('societe',auth()->user()->societe)->where('id','>',$id)->orderBy('id','asc')->count();
        if($testSuivant > 0){
            $suivant = AvancePret::where('societe',auth()->user()->societe)->where('id','>',$id)->orderBy('id','asc')->first();
            $next = $suivant->id;             
        ...
        if($test > 0){
            ...
            if($autoriser == 1){ 
                ...
                if($test_user > 0){                    
                    ...
                    $test_avances = AvancePret::where('societe',auth()->user()->societe)->where('id', $this->ids)->count();
                    if($test_avances > 0){
