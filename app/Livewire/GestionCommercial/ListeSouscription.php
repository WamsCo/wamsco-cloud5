...
class ListeSouscription extends Component
{
    ...
    // pour creer le role automatique
        ...
        // CRM
        public $consulter_opportunite = true, $creer_opportunite = true, $detail_opportunite = true, $modifier_opportunite = true, $assigner_opportunite = true, $supprimer_opportunite = true;                          
    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){ 
            if($mod_gestion_commercial == 1){             
                $title = 'Liste souscription | WamsCo';
                $module = 'Gestion commerciale';
                $title_fils = 'Liste souscription';
                $lien = 'liste_souscription?active=16&champ=1-3';
                $active = request('active');
                $champ = request('champ');
                $choix = request('choix'); 
                $dateJour = date('Y-m-d');                
                
                $liste_entit = Entite::where('recommandation',auth()->user()->email)->where('enseigne','like','%'.$this->query.'%')->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage); 
                $entite_count = $liste_entit->count(); 

                $resultat = Entite::where('recommandation',auth()->user()->email)->get();  
                $nbreTotalEntite = $resultat->count();     

                $derniereActivite = Entite::where('recommandation',auth()->user()->email)->latest('updated_at')->first();                
                ...
                return view('livewire.gestion-commercial.liste-souscription',compact('title_fils','module','lien','dateJour','liste_entit','entite_count','nbreTotalEntite','derniereActivite','log','logCount'))->layout('components.layouts.app',compact('title','active','champ','choix','entite_mod','dateJour','soldeClient','nbjoursRestant')); 
            }
            else{
                $title = 'Bienvenue'; 
                $module = 'Home';
    ...
    public function store(){ 
        ...
        if($test > 0){
            $role = Role::where('societe_id',auth()->user()->societe_id)->where('nom',auth()->user()->type_user)->get();
            $autoriser = $role[0]->creer_souscription;
            if($autoriser == 1){ 
                try {
                    $dateJour = date('Y-m-d'); 
                    $nbrJour = 14;            
                    ...
                    if($this->choix_plan == 'Standard Mensuel' || $this->choix_plan == 'Standard Annuel') {                   
                        $this->nbre_user_max = 3;
                        $this->nombre_users = 1; 
                        $this->taux_commission = 30; // en %
                    }
                    else{
                        $this->nbre_user_max = 1;
                        $this->nombre_users = 1;
                        $this->taux_commission = 20; // en %
                    }

                    $this->nombre_societe = 1;
                    $this->montant_paye = 0;            
                    $this->recommandation = auth()->user()->email;
                    // $this->taux_commission = 0;
                    $this->montant_commission = 0;
                    $this->etat_commission = 'Non payé';
                    $solde = 0;
                    $sexe = 'Non-défini';
                    $salarie = 0;
                    ...
                    $entit = Entite :: create(['solde'=>$solde,'enseigne'=>$this->societe,'societe_mere'=>$this->societe,'raison_sociale'=>$this->societe,'slug'=>$slug,'responsable_societe'=>$this->nom_utilisateur,'telephone'=>$this->telephone,'ville'=>$this->ville,'adresse'=>$this->adresse,'email'=>$this->email,
                            ...
                            'taux_commission'=>$this->taux_commission,'montant_commission'=>$this->montant_commission,'etat_commission'=>$this->etat_commission,'registre_com'=>$this->registre_commerce,'niu'=>$this->niu,'code_postal'=>$this->code_postal,'site_web'=>$this->site_web,
                            'condition_vente'=>$this->condition_vente,'nom_user'=>auth()->user()->name,'user_id'=>auth()->user()->id]); 
                    ...
                    $entrep = Entrepot::create(['nom'=>$nom_entrepot,'reference'=>$reference,'active'=>1,'description'=>$nom_entrepot,
                    ...
                    $objectif_point = 0;
                    $solde = 0;
                    Tier::create(['nom'=>$nom_tier,'raison_sociale'=>$nom_tier,'solde'=>$solde,'type_tiers'=>$type_tiers,'etat'=>1,'telephone'=>$this->telephone,'email'=>$this->email,
                    ...
                    return redirect('/liste_souscription?active=16&champ=1-3');
                }
