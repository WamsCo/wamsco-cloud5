    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){    
            if($mod_administration == 1){        
                ...
                if(empty($this->query)){
                    $liste_soldeClient = soldeClient::where('enseigne',auth()->user()->societe)->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage); 
                }
                else{
                    $liste_soldeClient = soldeClient::where('enseigne',auth()->user()->societe)->where('designation','like','%'.$this->query.'%')->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage); 
                }
