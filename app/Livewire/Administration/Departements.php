    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_administration == 1){
                ...
                $departement = Departement :: where('societe',auth()->user()->societe)->where('nom_departement','like','%'.$this->query.'%')->orderBy('id','desc')->paginate($this->parPage); 
                $departementcount = $departement->count(); 
                
                $resultat = Departement :: where('societe',auth()->user()->societe)->get();  
                $nbreTotalDeparte = $resultat->count(); 

                $derniereActivite = Departement::where('societe',auth()->user()->societe)->latest('updated_at')->first(); 
    ...
    public function store(){
        ...
        if($test > 0){
            ...
            if($autoriser == 1){ 
                ...
                Departement :: create(['nom_departement'=>$this->nom_departement,'description'=>$this->description,'societe'=>auth()->user()->societe,'nom_user'=>auth()->user()->name,'user_id'=>auth()->user()->id]);
                
                $dernier_id = Departement::where('societe',auth()->user()->societe)->where('user_id',auth()->user()->id)->latest()->first()->id; 
        ...
        if($test > 0){
            ...
            if($autoriser == 1){  
                if($this->ids){
                    ...
                    Departement::find($this->ids)->update(['nom_departement'=>$this->nom_departement,'description'=>$this->description,'societe'=>auth()->user()->societe,'nom_user'=>auth()->user()->name,'user_id'=>auth()->user()->id]);
