    ...
    public function render(){
        ...
        if($dateJour <= $jourValid){
            if($mod_banque_caisse == 1){
                ...
                $compte_banque = CompteBancaire::where('societe_id',auth()->user()->societe_id)->where('etat',1)->orderBy('nom_compte_bancaire','ASC')->get();
                    // $compteBanqueCount = $compte_banque->count();

                $resultat = CompteBancaire::where('societe_id',auth()->user()->societe_id)->where('etat',1)->get();  
    ...
    public function valider(){
        ...
        if($test > 0){
            ...
            if($autoriser == 1){    
                    ...
                    $test_compte = CompteBancaire::where('societe_id',auth()->user()->societe_id)->where('id',$this->compte_source)->count();    
                    if($test_compte > 0){
                        $compte = CompteBancaire::where('societe_id',auth()->user()->societe_id)->where('id',$this->compte_source)->first();               
                        $this->ids = $compte->id;
                        $this->compte_bancaire = $compte->nom_compte_bancaire;
                        $this->solde_source = $compte->solde;

                        $compteDesti = CompteBancaire::where('societe_id',auth()->user()->societe_id)->where('id',$this->compte_destination)->first();               
                    ...
                    CompteBancaire::where('societe_id',auth()->user()->societe_id)->where('id',$this->ide)->update(['solde'=>$solde_net_destination]); 
                    ...
                    EcritureBancaire::create(['id_compte_bancaire'=>$this->ids,'nom_compte_bancaire'=>$this->compte_bancaire,'reference'=>$reference,
                                    ...
                                    'type_paiement'=>$type_paiement,'id_type_paiement'=>0,'statut'=>$statut,'societe'=>auth()->user()->societe,'societe_id'=>auth()->user()->societe_id,'nom_user'=>auth()->user()->name,'user_id'=>auth()->user()->id]);  
