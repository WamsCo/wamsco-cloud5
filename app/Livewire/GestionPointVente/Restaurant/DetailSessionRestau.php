...
class DetailSessionRestau extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_pointe_vente == 1){
                $title = 'Session Restaurant | WamsCo';
                $module = 'Gestion point vente';
                $title_fils = 'Session Restaurant';
                $lien = 'restau_sessions';
                ...
                if($test > 0){
                    ...
                    //                 position:'top-end',
                    //             );
                    //             $this->redirect('/restau_sessions?active=5&champ=2-1&choix=1', navigate: true);
                    //         }
                    //     }
                    // }
                    // else{             
                ...
                return view('livewire.gestion-point-vente.restaurant.session.detail-session-restau',compact('title_fils','module','lien','dateJour','factClient_entete','factClientEnteteCount',
    ...
    public function ouvertureCaisse(){
        $this->validate([            
         ...
         $this->redirect('/pipeline_restau?id='.$this->ids.'&ref='.$this->session_id.'&active=5&champ=2-1&choix=1', navigate: true);
    } 
    ...
    public function closeSession(int $id){        
       ...
       if($verifie_id == auth()->user()->id){       
            ...
            if($test == 0){ 
                    ...
                    $this->redirect('/detail_restau_session?id='.$this->ids.'&ref='.$this->session_id.'&active=5&champ=2-1&choix=1', navigate: true);
                    $this->dispatch('alert',                    
    ...
    public function precedant(int $id){             
        ...
        if($testPrecedant > 0){ 
            ...
            $this->redirect('/detail_restau_session?id='.$id.'&active=5&champ=2-1&choix=1', navigate: true);
        }           
    }    
