    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_pointe_vente == 1){
                ...
                $table = TableRestau :: where('societe',auth()->user()->societe)->where('nom_table','like','%'.$this->query.'%')->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage); 
                ...
                $resultat = TableRestau::where('societe',auth()->user()->societe)->get();  
                $nbreTotalEspaceRestau = $resultat->count();     

                $derniereActivite = TableRestau::where('societe',auth()->user()->societe)->latest('updated_at')->first(); 
        ...
        if($test > 0){
            ...
            if($autoriser == 1){                                    
                ...
                if($test_espace > 0){
                    ...
                    'step'=>$this->evolution,'position'=>$position,'societe'=>auth()->user()->societe,'nom_user'=>auth()->user()->name,'user_id'=>auth()->user()->id]);
