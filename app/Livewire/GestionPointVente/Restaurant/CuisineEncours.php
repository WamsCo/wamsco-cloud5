...
class CuisineEncours extends Component
{
    use WithPagination;
    use WithFileUploads;
    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_cuisine == 1){
                ...
                // // statut en cours
                $emplacement_encour = TableRestau::where('societe_id',auth()->user()->societe_id)->where('utiliser','Oui')->where('statut','En cours')->orderBy('updated_at', 'DESC')->get(); 
                $emplacement_encourCount = $emplacement_encour->count();

                // statut terminer
                $emplacement_terminerCount = TableRestau::where('societe_id',auth()->user()->societe_id)->where('utiliser','Oui')->where('statut','Terminer')->orderBy('updated_at', 'DESC')->count(); 
                $cmdAttenteLigne = RestauCommandeAttenteLigne::where('societe_id',auth()->user()->societe_id)->get();            

                // $page = 'SessionPos'; // Pour evenement lie
                // $log = LogActivityModel::where('societe_id',auth()->user()->societe_id)->where('id_activite', $this->ids)->where('page', $page)->limit(10)->orderBy('id','desc')->get();
                // $logCount = $log->count();            
                ...
                return view('livewire.gestion-point-vente.restaurant.cuisine.cuisine-encours',compact('title_fils','module','lien','dateJour','emplacement_allCount','emplacement_prepaCount','emplacement_encour','emplacement_encourCount',
