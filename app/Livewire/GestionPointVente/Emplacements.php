...
class Emplacements extends Component
{
    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_pointe_vente == 1){
                ...
                $emplacement = Emplacement :: where('societe_id',auth()->user()->societe_id)->where('nom_emplacement','like','%'.$this->query.'%')->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage); 
                ...
                $resultat = Emplacement::where('societe_id',auth()->user()->societe_id)->get();  
                $nbreTotalEmplacement = $resultat->count();     

                $derniereActivite = Emplacement::where('societe_id',auth()->user()->societe_id)->latest('updated_at')->first();

                $page = 'Emplacement'; // Pour evenement lie
                $log = LogActivityModel::where('societe_id',auth()->user()->societe_id)->where('page', $page)->limit(8)->orderBy('id','desc')->get();
                $logCount = $log->count();   
    ...
    public function store(){
        ...
        if($test > 0){
            ...
            $utiliser = 'Non';
            if($autoriser == 1){ 
                $emplace = Emplacement :: create(['nom_emplacement'=>$this->nom_emplacement,'description'=>$this->description,'utiliser'=>$utiliser,'societe'=>auth()->user()->societe,'societe_id'=>auth()->user()->societe_id,'user_id'=>auth()->user()->id,'nom_user'=>auth()->user()->name]);
        ...
        if($test > 0){
            ...
            if($autoriser == 1){  
                if($this->ids){
                    ...
                    Emplacement::find($this->ids)->update(['nom_emplacement'=>$this->nom_emplacement,'description'=>$this->description,'societe'=>auth()->user()->societe,'societe_id'=>auth()->user()->societe_id,'nom_user'=>auth()->user()->name,'user_id'=>auth()->user()->id]);
