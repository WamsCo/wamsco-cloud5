    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_cuisine == 1){
                ...
                // statut A préparer
                $emplacement_prepa = TableRestau::where('societe',auth()->user()->societe)->where('utiliser','Oui')->where('statut','A préparer')->orderBy('updated_at', 'DESC')->get(); 
                $emplacement_prepaCount = $emplacement_prepa->count();

                // statut en cours
                $emplacement_encour = TableRestau::where('societe',auth()->user()->societe)->where('utiliser','Oui')->where('statut','En cours')->orderBy('updated_at', 'DESC')->get(); 
                $emplacement_encourCount = $emplacement_encour->count();
                ...
                return view('livewire.gestion-point-vente.cuisine_pos',compact('title_fils','module','lien','dateJour','emplacement','emplacement_allCount','emplacement_prepa','emplacement_prepaCount','emplacement_encour','emplacement_encourCount',
                'emplacement_terminer','emplacement_terminerCount','cmdAttenteLigne'))->layout('components.layouts.app_pos',compact('title','active','champ','choix','entite_mod','dateJour','soldeClient','nbjoursRestant','id','ref'));
            }
            else{
                $title = 'Bienvenue'; 
                $module = 'Home';
