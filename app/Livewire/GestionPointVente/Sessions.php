        ...
        if($dateJour <= $jourValid){
            if($mod_pointe_vente == 1){
                ...
                if(empty($this->parUser)){
                    $session = SessionPos::where('societe_id',auth()->user()->societe_id)->where('nom_user','like','%'.$this->query.'%')->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                }
                else{
                    $session = SessionPos::where('societe_id',auth()->user()->societe_id)->where('nom_user','like','%'.$this->query.'%')->where('user_id', $this->parUser)->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                }
                ...
                $resultat = SessionPos::where('societe_id',auth()->user()->societe_id)->get();  
                $nbreTotalSessionPos = $resultat->count();     

                $derniereActivite = SessionPos::where('societe_id',auth()->user()->societe_id)->latest('updated_at')->first();
    ...
    public function store(){
        ...
        if($test > 0){
            ...
            if($autoriser == 1){   
                ...
                $countSession = SessionPos::where('societe_id',auth()->user()->societe_id)->where('user_id',auth()->user()->id)->count(); 
                if($countSession == 0){  
                     ...
                     // $token_ok = 'POS/'.$dates;
                     $sessions = SessionPos :: create(['session_id'=>$token_ok,'nom_point_vente'=>$nom_point_vente,'solde_initial'=>$solde_initial,
                                 'solde_final'=>$solde_final,'solde_cloture_theorique'=>$solde_cloture_theorique,'etat'=>$etat,'societe'=>auth()->user()->societe,'societe_id'=>auth()->user()->societe_id,'nom_user'=>auth()->user()->name,'user_id'=>auth()->user()->id]);
 
                     // ceci recupere le dernier enregistrement cree a l'instant
                     $dernier_id = $sessions->id; 
                    ...
                    $dernier_id = SessionPos::where('societe_id',auth()->user()->societe_id)->where('user_id',auth()->user()->id)->latest()->first()->id; 
                    $test_session = SessionPos::where('societe_id',auth()->user()->societe_id)->where('id',$dernier_id)->first();
