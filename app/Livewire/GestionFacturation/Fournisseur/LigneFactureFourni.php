...
class LigneFactureFourni extends Component
{
    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_facturation == 1){
                $title = 'Ligne de facture fournisseur | WamsCo';
                $module = 'Gestion facturation';
                $title_fils = 'Ligne de facture fournisseur';
                $lien = 'ligne_facture_fourni';
                ...
                $fact_fourni = factureFournisseurLigne::where('societe_id',auth()->user()->societe_id)->where('code_facture','like','%'.$this->parFact.'%')->where('nom_fournisseur','like','%'.$this->query.'%')->where('id_produit','like','%'.$this->parNomProduit.'%')->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                $factFourniCount = $fact_fourni->count();
                $QuantiteTotal = $fact_fourni->sum('quantite');
                $montantTTC = $fact_fourni->sum('montant_ttc');
                $montantTRemise = $fact_fourni->sum('montant_remise');
                $montantTmarge = $fact_fourni->sum('marge');  
                
                // pour les KPI
                $resultat = factureFournisseurLigne::where('societe_id',auth()->user()->societe_id)->get();  
                ...
                $derniereActivite = factureFournisseurLigne::where('societe_id',auth()->user()->societe_id)->latest('updated_at')->first();
                ...
                return view('livewire.gestion-facturation.fournisseur.ligne-facture-fourni',compact('title_fils','module','lien','dateJour','produit','fact_fourni','factFourniCount','QuantiteTotal','montantTTC','montantTmarge',
