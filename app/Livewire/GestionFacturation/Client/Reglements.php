    ...
    public function render(){
        ...
        if($dateJour <= $jourValid){
            if($mod_facturation == 1){
                ...
                $Reglement_client = Reglement::where('societe',auth()->user()->societe)->where('ref_reglement','like','%'.$this->parRef.'%')->where('code_facture','like','%'.$this->parFact.'%')->where('nom_client','like','%'.$this->query.'%')->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                $regleClientCount = $Reglement_client->count();
                $montantTregler = $Reglement_client->sum('montant_regler'); 

                $resultat = Reglement::where('societe',auth()->user()->societe)->get();  
                $nbreTotalReglement = $resultat->count();  
                $montantTregler_all = $resultat->sum('montant_regler');   

                $derniereActivite = Reglement::where('societe',auth()->user()->societe)->latest('updated_at')->first();
