...
class LigneCommandeFourni extends Component
{
    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_cmd == 1){
                $title = 'Ligne de commande fournisseur | WamsCo';
                $module = 'Gestion commande';
                $title_fils = 'Ligne de commande fournisseur';
                $lien = 'ligne_cmd_fourni';
                ...
                $cmd_fourni = CommandeFournisseurLigne::where('societe_id',auth()->user()->societe_id)->where('nom_fournisseur','like','%'.$this->query.'%')->where('id_produit','like','%'.$this->parNomProduit.'%')->where('code_commande','like','%'.$this->parCmd.'%')->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                $cmdFourniCount = $cmd_fourni->count();
                $QuantiteTotal = $cmd_fourni->sum('quantite');
                $montantTTC = $cmd_fourni->sum('montant_ttc');
                $montantTRemise = $cmd_fourni->sum('montant_remise');
                $montantTmarge = $cmd_fourni->sum('marge'); 
                
                 // KPI
                $resultat = CommandeFournisseurLigne::where('societe_id',auth()->user()->societe_id)->get();  
                $nbreTotalLigne = $resultat->count(); 
                ...
                $derniereActivite = CommandeFournisseurLigne::where('societe_id',auth()->user()->societe_id)->latest('updated_at')->first();
                ...
                return view('livewire.gestion-commande.fournisseur.ligne-commande-fourni',compact('title_fils','module','lien','dateJour','produit','cmd_fourni','cmdFourniCount','QuantiteTotal','montantTTC','montantTmarge',
                'montantTRemise','nbreTotalLigne','montantTTC_all','montantTRemise_All','montantTmarge_All','derniereActivite','log','logCount'))->layout('components.layouts.app',compact('title','active','champ','choix','entite_mod','dateJour','soldeClient','nbjoursRestant'));
            }
            else{
                $title = 'Bienvenue'; 
                $module = 'Home';
