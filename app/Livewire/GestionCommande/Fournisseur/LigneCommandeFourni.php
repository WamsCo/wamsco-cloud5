    ...
    public function render()
    {
        ...
        if($dateJour <= $jourValid){
            if($mod_cmd == 1){
                ...
                $cmd_fourni = CommandeFournisseurLigne::where('societe',auth()->user()->societe)->where('nom_fournisseur','like','%'.$this->query.'%')->where('id_produit','like','%'.$this->parNomProduit.'%')->where('code_commande','like','%'.$this->parCmd.'%')->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                ...
                $montantTmarge = $cmd_fourni->sum('marge'); 
                 ...
                 // KPI
                $resultat = CommandeFournisseurLigne::where('societe',auth()->user()->societe)->get();  
                ...
                $derniereActivite = CommandeFournisseurLigne::where('societe',auth()->user()->societe)->latest('updated_at')->first();
