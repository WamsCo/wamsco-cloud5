    ...
    public function render(){
        ...
        if($dateJour <= $jourValid){
            if($mod_cmd == 1){
                ...
                $montantTTC = $cmd_fournisseur->sum('montant_ttc');
                $montantTrecu = $cmd_fournisseur->sum('montant_recu');
                $montantTreste_Percevoir = $cmd_fournisseur->sum('reste_a_percevoir');
                ...
                return view('livewire.gestion-commande.fournisseur.commande-fournisseur',compact('title_fils','module','lien','dateJour','cmd_fournisseur','cmdFournisseurCount','montantTTC','montantTrecu','montantTreste_Percevoir','montantTmarge',
       ...
       $test = Role::where('societe',auth()->user()->societe)->where('nom',auth()->user()->type_user)->count();
        if($test > 0){
            ...
            if($autoriser == 1){   
                ...
                LogActivity::addToLog('commande fournisseur » '.$token_ok.' créé', $id_activite, $page); 
                $this->dispatch('alert',                    
                ...
                $this->redirect('/nouveau_cmd_fourni?id='.$dernier_id.'&ref='.$token_ok.'&active=6&champ=2-1&choix=1', navigate: true);               
            }
            else{                 
                $this->dispatch('alert',                    
            ...
            if($autoriser == 1){   
                if($id){                   
                    ...
                    if($test_regle == 0){
                        ...
                        CommandeFournisseurLigne::where('id_commande_fournisseur_entete',$id)->delete();
                        LogActivityModel::where('id_activite',$id)->where('page',$page)->delete();
                        $id_activite = $dernier_id;
                        LogActivity::addToLog('Commande fournisseur supprimée définitivement', $id_activite, $page);
                        $this->dispatch('alert',                    
