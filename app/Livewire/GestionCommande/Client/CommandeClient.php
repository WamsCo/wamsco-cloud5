        ...
        if($dateJour <= $jourValid){
            if($mod_cmd == 1){
                ...
                if(!empty($this->parEtat)){
                    $cmd_client = CommandeClientEntete::where('societe_id',auth()->user()->societe_id)->where('nom_client','like','%'.$this->query.'%')->where('code_commande','like','%'.$this->parCmd.'%')->where('etat',$this->parEtat)->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                }
                else{
                    $cmd_client = CommandeClientEntete::where('societe_id',auth()->user()->societe_id)->where('nom_client','like','%'.$this->query.'%')->where('code_commande','like','%'.$this->parCmd.'%')->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                }
                ...
                // pour les KPI
                $resultat = CommandeClientEntete::where('societe_id',auth()->user()->societe_id)->get();  
                ...
                $derniereActivite = CommandeClientEntete::where('societe_id',auth()->user()->societe_id)->latest('updated_at')->first();
       ...
       $test = Role::where('societe_id',auth()->user()->societe_id)->where('nom',auth()->user()->type_user)->count();
        if($test > 0){
            ...
            if($autoriser == 1){   
                ...
                $cmdcltEntet = CommandeClientEntete :: create(['code_commande'=>$token_ok,'nom_client'=>$client,'id_client'=>$id_client,'date_commande'=>$date_commande,'date_livraison'=>$date_livraison,
        ...
        if($test > 0){ 
            ...
            if($autoriser == 1){   
                if($id){
                    ...
                    $test_regle = factureClientEntete::where('societe_id',auth()->user()->societe_id)->where('code_facture',$code_fact)->count();
                    if($test_regle == 0){
