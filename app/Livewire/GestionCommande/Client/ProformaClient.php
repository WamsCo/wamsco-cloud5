...
class ProformaClient extends Component
{
        ...
        if($dateJour <= $jourValid){
            if($mod_cmd == 1){
                ...
                if(!empty($this->parEtat)){
                    $profor_client = ProformaClientEntete::where('societe',auth()->user()->societe)->where('nom_client','like','%'.$this->query.'%')->where('code_proforma','like','%'.$this->parProf.'%')->where('etat',$this->parEtat)->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                }
                else{
                    $profor_client = ProformaClientEntete::where('societe',auth()->user()->societe)->where('nom_client','like','%'.$this->query.'%')->where('code_proforma','like','%'.$this->parProf.'%')->whereBetween('created_at',[$start, $end])->orderBy($this->orderField, $this->orderDirection)->paginate($this->parPage);
                }
                ...
                // pour les KPI
                $resultat = ProformaClientEntete::where('societe',auth()->user()->societe)->get();  
                ...
                $derniereActivite = ProformaClientEntete::where('societe',auth()->user()->societe)->latest('updated_at')->first();
       ...
       $test = Role::where('societe',auth()->user()->societe)->where('nom',auth()->user()->type_user)->count();
        if($test > 0){
            ...
            if($autoriser == 1){   
                ...
                ProformaClientEntete :: create(['code_proforma'=>$token_ok,'nom_client'=>$client,'id_client'=>$id_client,'date_proforma'=>$date_proforma,'date_livraison'=>$date_livraison,
        ...
        if($test > 0){ 
            ...
            if($autoriser == 1){   
                if($id){
                    ...
                    $test_regle = CommandeClientEntete::where('societe',auth()->user()->societe)->where('code_commande',$code_cmd)->count();
                    if($test_regle == 0){
    ...
    public function detailCmd(int $idx, $codeCmd_prof){
        ...
        $test_commande = CommandeClientEntete::where('societe',auth()->user()->societe)->where('id',$idx)->count();    
        if($test_commande > 0){
