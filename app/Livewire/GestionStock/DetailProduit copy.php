...
class DetailProduit extends Component
{
        ...
        if($dateJour <= $jourValid){ 
            if($mod_gestion_stock == 1){           
                ...
                if($this->ids){
                    ...
                    $produit = Produit::where('societe',auth()->user()->societe)->where('id',$this->ids)->get(); 
                    $stock = Stock::where('societe',auth()->user()->societe)->where('id_produit',$this->ids)->where('type_produit','Produit')->get();  
                    ...
                    $mouvement = Mouvement::where('societe',auth()->user()->societe)->where('id_produit',$this->ids)->orderBy($this->orderField, $this->orderDirection)->limit($this->affiche)->get(); 
                    $mouvCountAfficher = $mouvement->count();
                    $mouvementCount = Mouvement::where('societe',auth()->user()->societe)->where('id_produit',$this->ids)->count();                
                    $listePrixVente = PrixVente::where('societe',auth()->user()->societe)->where('id_produit',$this->ids)->orderBy($this->orderField, $this->orderDirection)->limit($this->affiche)->get(); 
                    $listePrixVenteCount = $listePrixVente->count();
                    
                    $page = 'Produits'; // Pour evenement lie
                    $log = LogActivityModel::where('user_societe',auth()->user()->societe)->where('id_activite', $this->ids)->where('page', $page)->limit(20)->orderBy('id','desc')->get();
                    $logCount = $log->count();
                }
                else{
                    // ceci quand on click sur Edit() ou modifier
                    $produit = Produit::where('societe',auth()->user()->societe)->where('id',$this->id)->get();  
                    $stock = Stock::where('societe',auth()->user()->societe)->where('id_produit',$this->id)->where('type_produit','Produit')->get();  
                    $stockCount = $stock->count();
                    $stockPieceCount = $stock->sum('quantite'); 
                    $valeurVenteTotal = $stock->sum('valeur_vente_total'); 
                    $valorisation_achat_total = $stock->sum('valorisation_achat_total');
                    
                    $mouvement = Mouvement::where('societe',auth()->user()->societe)->where('id_produit',$this->id)->orderBy($this->orderField, $this->orderDirection)->limit($this->affiche)->get(); 
                    $mouvCountAfficher = $mouvement->count();
                    $mouvementCount = Mouvement::where('societe',auth()->user()->societe)->where('id_produit',$this->id)->count();
                    $listePrixVente = PrixVente::where('societe',auth()->user()->societe)->where('id_produit',$this->id)->orderBy($this->orderField, $this->orderDirection)->limit($this->affiche)->get(); 
                    $listePrixVenteCount = $listePrixVente->count();
                    
                    $page = 'Produits'; // Pour evenement lie
                    $log = LogActivityModel::where('user_societe',auth()->user()->societe)->where('id_activite', $this->id)->where('page', $page)->limit(20)->orderBy('id','desc')->get();
                    $logCount = $log->count();
                }
    ...
    public function update(){
        ...
        if($this->type_produit == 'Produit'){
            $this->validate([                      
                ...
                'code_barre' => ['nullable','string','max:255', Rule::unique('produits', 'code_barre')->where(fn($query) => $query->where('societe', auth()->user()->societe))->ignore($this->id),],     
        ...
        if($test > 0){
            ...
            if($autoriser == 1){      
                ...
                if($this->entrepot > 0){
                    ...
                    $test_existe =  Stock::where('societe',auth()->user()->societe)->where('id_produit',$this->id)->where('id_entrepot',$this->entrepot)->count();
                    if($test_existe == 0){ 
                        Stock::create(['id_entrepot'=>$this->entrepot,'nom_produit'=>$this->nom_produit,'id_produit'=>$this->id,'reference'=>$this->reference,'code_barre'=>$this->code_barre,'categorie'=>$this->categorie,'type_produit'=>$this->type_produit,'nature_produit'=>$this->nature_produit,'quantite'=>$quantite,
                ...
                Stock::where('id_produit',$this->id)->update(['nom_produit'=>$this->nom_produit,'reference'=>$this->reference,'code_barre'=>$this->code_barre,'categorie'=>$this->categorie,'type_produit'=>$this->type_produit,'nature_produit'=>$this->nature_produit,'categorie'=>$this->categorie,'etat'=>$this->etat,]);   
        ...
        if($test > 0){
            ...
            if($autoriser == 1){ 
                ...
                if($this->id){     
                    ...
                    $this->redirect('/detail_product?id='.$this->id, navigate: true);
                }
            }
            else{                 
                $this->dispatch('alert',                    
    ...
    public function corriger(){
        ...
        if($test > 0){
            ...
            if($autoriser == 1){                 
                    $produit = Produit::where('societe',auth()->user()->societe)->where('id',$this->id)->first();
                    ...
                    $entrepo = Entrepot::where('societe',auth()->user()->societe)->where('id',$this->nom_entrepot)->first();
                    $this->id_entrepot = $entrepo->id;
                    $this->nom_entrepot = $entrepo->nom;
                   
                    if($this->sens_stock == 'Ajouter'){                    
                        
                        $test = Stock::where('societe',auth()->user()->societe)->where('id_entrepot',$this->id_entrepot)->where('id_produit',$this->id)->count(); 
                        if($test > 0){
                            $stoc = Stock::where('societe',auth()->user()->societe)->where('id_entrepot',$this->id_entrepot)->where('id_produit',$this->id)->first();                         
                            $id_stock = $stoc->id;
                    ...
                    elseif($this->sens_stock == 'Supprimer'){ 
                        ...
                        $test = Stock::where('societe',auth()->user()->societe)->where('id_entrepot',$this->id_entrepot)->where('id_produit',$this->id)->count(); 
                        if($test > 0){
                            $stoc = Stock::where('societe',auth()->user()->societe)->where('id_entrepot',$this->id_entrepot)->where('id_produit',$this->id)->get();                         
                            $id_stock = $stoc[0]->id;   
    ...
    public function precedant(int $id){ 
        $testPrecedant = Produit::where('societe',auth()->user()->societe)->where('id','<',$id)->orderBy('id','desc')->count();
        if($testPrecedant > 0){ 
            $precedant = Produit::where('societe',auth()->user()->societe)->where('id','<',$id)->orderBy('id','desc')->first();        
    ...
    public function suivant(int $id){    
        ...
        $testSuivant = Produit::where('societe',auth()->user()->societe)->where('id','>',$id)->orderBy('id','asc')->count();
        if($testSuivant > 0){
            $suivant = Produit::where('societe',auth()->user()->societe)->where('id','>',$id)->orderBy('id','asc')->first();
            $next = $suivant->id;             
    ...
    public function genererCodeBarre(){    
        $this->validate([              
            'code_barre' => ['nullable','string','max:255',
             Rule::unique('produits', 'code_barre')->where(fn($query) => $query->where('societe', auth()->user()->societe))->ignore($this->id),]
        ]);         
