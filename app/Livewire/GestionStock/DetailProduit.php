        ...
        if($dateJour <= $jourValid){ 
            if($mod_gestion_stock == 1){           
                ...
                if($this->ids){
                    ...
                    $stock = Stock::where('societe',auth()->user()->societe)->where('id_produit',$this->ids)->where('type_produit','Produit')->where('etat',1)->get();  
                    ...
                    $stock = Stock::where('societe',auth()->user()->societe)->where('id_produit',$this->id)->where('type_produit','Produit')->where('etat',1)->get();  
                    $stockCount = $stock->count();
                    $stockPieceCount = $stock->sum('quantite'); 
                    $valeurVenteTotal = $stock->sum('valeur_vente_total'); 
                    $valorisation_achat_total = $stock->sum('valorisation_achat_total');
        ...
        if($test > 0){
            ...
            if($autoriser == 1){      
                ...
                if($this->type_produit == 'Produit'){
                    ...
                    $stockLigne = Stock::where('societe',auth()->user()->societe)->where('id_produit',$this->id)->get(); 
