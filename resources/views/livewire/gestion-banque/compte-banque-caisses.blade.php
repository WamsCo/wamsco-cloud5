                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">                                                
                                                ...
                                                <div class="text-nowrap">
                                                    ...
                                                    <span class="d-block fw-semibold text-dark" style="font-size: 13px;">Actif : <span class="text-danger">{{number_format($nbreTotalCompteBancaireActif,0,',',' ')}}</span> / {{number_format($nbreTotalCompteBancaire,0,',',' ')}}</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home" title="Listing {{$title_fils}}"><i class="fas fa-university text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$banqueCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">   
                                            <button class="btn btn-sm btn-danger ms-auto" title="Cliquez pour enregistrer" data-toggle="tooltip" wire:click.prevent="store()"><i class="fa fa-plus-circle"></i> Nouvelle Banque/Caisse</button>                         
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <h6><a href="{{asset('exporter-compte_banq')}}" class="btn btn-sm btn-success mt-2 d-none d-md-block"><i class="fa-solid fa-file-export"></i> Exporter Excel</a></h6>
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            <div>
                                                                ...
                                                                <input type="search" wire:model.live="query" id="query" class="form-control bordure" placeholder="Rechercher compte">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('nom_compte_bancaire')">Comptes bancaires <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('type_compte')">Type <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('num_compte')">Num. compte <i class="fa fa-arrow-down-short-wide"></i></th>  
                                                                    <th class="fond_entete_table text-end pointer fw-semibold" wire:click="setOrderField('solde')">Solde <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table text-center pointer fw-semibold">Devise</th> 
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('nom_banque')">Banque <i class="fa fa-arrow-down-short-wide"></i></th>                  
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('nom_proprietaire')">Proprietaire <i class="fa fa-arrow-down-short-wide"></i></th>                     
                                                                    <th class="fond_entete_table text-center pointer" wire:click="setOrderField('etat')">Etat <i class="fa fa-arrow-down-short-wide"></i></th>                     
                                                            ...
                                                            <tbody>
                                                                @foreach($banque as $banques) 
                                                                    <tr>
                                                                        {{-- <td class="fw-semibold"><a href="detail_banque?id={{$banques->id}}" wire:navigate>{{$banques->reference}}</a></td>  --}}
                                                                        <td class="fw-semibold"><a href="detail_banque?id={{$banques->id}}&active=8&champ=1-1" wire:navigate class="text-bleu"><i class="fa fa-university text-danger"></i> {{substr($banques->nom_compte_bancaire,0,52) > substr($banques->nom_compte_bancaire,0,51) ? substr($banques->nom_compte_bancaire,0,52).'...': $banques->nom_compte_bancaire}}</a></td>                                                                      
                                                                        <td class="">{{$banques->type_compte}}</td>
                                                                        <td class="">{{$banques->num_compte}}</td>                                                                                        
                                                                        <td class="text-end fw-semibold @if($banques->solde > 0) text-vert @else text-danger @endif">{{number_format($banques->solde,0,',',' ')}}</td>                              
                                                                        <td class="text-center">{{$this->devise}}</td> 
                                                                        <td class="fw-semibold">{{$banques->nom_banque}}</td> 
                                                                        <td class="">{{$banques->nom_proprietaire}}</td>                                            
                                                                        @if($banques->etat == 1)
                                                                            <td class="pointer text-center" wire:click.prevent="changeEtat({{$banques->id}}, {{$banques->etat}})" title="Cliquez pour changer l'état"><span class="badge bg-success"><i class="fa fa-check-circle"></i></span></td>
                                                                        @else
                                                                            <td class="pointer text-center" wire:click.prevent="changeEtat({{$banques->id}}, {{$banques->etat}})" title="Cliquez pour changer l'état"><span class="badge bg-danger blink"><i class="fa fa-times-circle"></i></span></td>
                                                                        @endif                                                                
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($banques->updated_at))}}</td>
                                                                        <td class="text-end"><a href="detail_banque?id={{$banques->id}}" wire:navigate class="btn-outline-muted btn-xs" title="Cliquez pour modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-eye"></i></a></td>                                          
                                                                        <td class="taille_icon text-end">
                                                                            @if($confirmer === $banques->id)                                                               
                                                                                <a wire:click.prevent="supprimer({{$banques->id}})" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size: 8px;" title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer?</i></a>
                                                                            @else
                                                                                <a wire:click.prevent="confirmerDelete({{$banques->id}})" class="btn-outline-muted btn-xs pointer"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                            @endif
                                                                        </td> 
                                                                    </tr>
                                                                @endforeach
                                                        ...
                                                        <div class="bloc_pagination">{{$banque->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
