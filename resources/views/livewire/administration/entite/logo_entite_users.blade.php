			...
			<div class="modal-dialog modal-sm">
				<div class="modal-content">             
					...
					<div class="modal-body">   
                        <div class="card-body">
                            <div class="m-0">
                              <label for="logos" class="form-label">Sélectionnez une image (300x300)</label>
                              <div class="text-center p-3">                                   
                                    @if($logo)
                                        <img class="w-full h-32 rounded-3" style="width: 72px;border-radius: 50%;" src="storage/{{$logo}}">
                                    @else       
                                        <img class="w-full h-32 rounded-3" style="width: 72px;border-radius: 50%;" src="storage/default/image.png">                     
                                    @endif 
                              </div>
                              <input type="file" wire:model="logos" class="form-control @error('logos') is-invalid @enderror"  id="logos">
                              @error('logos')<div class="text-danger invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                        </div>
					</div>
                    <div class="d-flex align-items-center justify-content-between modal-footer py-1" style="background-color: #56585d; color:#ffffff;">					
                        <div class="">
                            <button type="submit" wire:click.prevent="valider_logo()" class="btn btn-sm btn-secondary fw-bold" title="Cliquez pour enregistrer" data-toggle="tooltip"><i class="fa fa-check"></i> Valider</button>
