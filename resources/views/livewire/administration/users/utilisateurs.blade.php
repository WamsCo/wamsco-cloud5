<div x-data="{selection: @entangle('selection').defer}">
                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">                                                
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total utilisateur</span>
                                                    <span class="d-block fw-semibold text-dark" style="font-size: 13px;">{{number_format($nbreTotalUtilisateur,0,',',' ')}}</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home" title="Liste des tieres : Prospects, Clients, Fournisseurs"><i class="fas fa-users text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$utilisateurCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">   
                                            <button data-bs-toggle="modal" data-bs-target="#createUserModal" title="Cliquez pour ajouter un nouveau tier" data-toggle="tooltip" class="btn btn-sm btn-danger ms-auto"><i class="fa fa-plus-circle"></i> Nouveau utilisateur</button>
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <h6><a href="{{asset('exporter-utilisateurs')}}" class="btn btn-sm btn-success mt-2 d-none d-md-block"><i class="fa-solid fa-file-export"></i> Exporter Excel</a></h6>
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div>
                                                                ...
                                                                <input type="search" wire:model.live="query" id="query" class="form-control bordure" placeholder="Rechercher par nom">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    <th class="fond_entete_table">Profil</th>
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('name')">Nom <i class="fa fa-arrow-down-short-wide"></i></th>   
                                                                    ...
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('type_user')">Rôle <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('date_valide')">Validité <i class="fa fa-arrow-down-short-wide"></i></th>                                            
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('societe')">Société <i class="fa fa-arrow-down-short-wide"></i></th>  
                                                            ...
                                                            <tbody>
                                                                @foreach($utilisateur as $utilisateurs) 
                                                                    <tr>                                            
                                                                        @if($utilisateurs->profil != null)
                                                                            <td><a class="text-bleu" href="detail_user?id={{$utilisateurs->id}}&soc={{$utilisateurs->societe}}&active=12&champ=1-1" wire:navigate><img class="img_produit" src="storage/{{$utilisateurs->profil}}"/></a></td>
                                                                        @else
                                                                            <td><a class="text-bleu" href="detail_user?id={{$utilisateurs->id}}&soc={{$utilisateurs->societe}}&active=12&champ=1-1" wire:navigate><img class="img_produit" src="storage/default/user_man.png"/></a></td>
                                                                        @endif                                                                   
                                                                        <td class="fw-bold"><a class="text-bleu" href="detail_user?id={{$utilisateurs->id}}&soc={{$utilisateurs->societe}}&active=12&champ=1-1" wire:navigate><i class="fa fa-user-circle"></i> {{substr($utilisateurs->name,0,52) > substr($utilisateurs->name,0,51) ? substr($utilisateurs->name,0,52).'...': $utilisateurs->name}}</a></td>                                                                      
                                                                        <td class="fw-semibold">
                                                                            @if($utilisateurs->sexe == "Masculin")
                                                                                <span class="text-success">{{$utilisateurs->sexe}}</span>
                                                                            @else 
                                                                                <span class="text-danger">{{$utilisateurs->sexe}}</span>
                                                                            @endif
                                                                        </td> 
                                                                        <td class="fw-bold">{{$utilisateurs->email}} <span class="fw-semibold">@if($utilisateurs->confirmer == 1) (<span class="text-success" title="Email confirmé" style="font-size: 11px;"><i class="fa-solid fa-thumbs-up"></i></span>) @else <span>(<span class="text-danger blink" title="Email Non-confirmé" style="font-size: 11px;"><i class="fa-solid fa-thumbs-down"></i></span>) </span> @endif</span></td> 
                                                                        <td class="fw-semibold">{{$utilisateurs->telephone}}</td> 
                                                                        <td class="text-start">{{$utilisateurs->type_user}}</td>
                                                                        <td class="fw-semibold">
                                                                            @php
                                                                                $nbjoursRestant = round((strtotime($utilisateurs->date_valide) - strtotime($dateJour))/(60*60*24));
                                                                            @endphp 
                                                                            {{date('d-m-Y', strtotime($utilisateurs->date_valide))}}  
                                                                            @if($nbjoursRestant > 0 )
                                                                                <span class="text-bleu">({{$nbjoursRestant}} Jours)</span>
                                                                            @else
                                                                                (<span class="text-danger" style="font-size: 10px;">Dépassée</span>)
                                                                            @endif
                                                                        </td> 
                                                                        <td class="">{{$utilisateurs->societe}}</td> 
                                                                        <td class="fw-semibold">{{$utilisateurs->societe_mere}}</td>                                            
                                                                        <td class="pointer text-center" wire:click.prevent="changeEtat({{$utilisateurs->id}}, '{{$utilisateurs->etat}}')" title="Cliquez pour changer l'état">
                                                                            @if($utilisateurs->etat == 1)
                                                                                <span class="badge bg-success"><i class="fa fa-check-circle"></i></span>
                                                                            @else
                                                                                <span class="blink badge bg-danger"><i class="fa fa-times-circle"></i></span>
                                                                            @endif                                            
                                                                        </td>
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($utilisateurs->created_at))}}</td>
                                                                        <td class=""><a href="detail_user?id={{$utilisateurs->id}}&active=12&champ=1-1" wire:navigate><i class="fa fa-eye btn-outline-muted" title="Cliquez pour voir ou modifier" data-toggle="tooltip"></i></a></td>
                                                                        <td class="taille_icon text-end">
                                                                            @if($userDispo > 1)
                                                                                @if($confirmer === $utilisateurs->id)
                                                                                    <a wire:click.prevent="supprimer({{$utilisateurs->id}})"><i class="fa fa-trash bg-red text-white w-32 px-1 py-1 rounded border pointer blink" style="font-size: 8px;" title="Cliquez pour confirmer la suppression" data-toggle="tooltip"> Confirmer?</i></a>
                                                                                @else
                                                                                    <a wire:click.prevent="confirmerDelete({{$utilisateurs->id}})" class="btn-outline-muted btn-xs pointer"><i class="fa fa-trash" title="Cliquez pour supprimer" data-toggle="tooltip"></i></a>
                                                                                @endif
                                                                            @endif
                                                                        </td> 
                                                                    </tr>                                        
                                                        ...
                                                        <div class="bloc_pagination">{{$utilisateur->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    @include('livewire.administration.users.create_utilisateur')    
</div>
