<div x-data="{selection: @entangle('selection').defer}">
                            ...
                            <div class="col-lg-12">
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">                            
                                            @if($societe_mere == $societe_filiale) 
                                                <a href="#" data-bs-toggle="modal" data-bs-target="#createFilialeModal" title="Cliquez pour créer une nouvelle société" data-toggle="tooltip" class="btn btn-sm btn-danger ms-auto"><i class="fa fa-plus-circle"></i> Nouvelle société</a>
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <div class="col-lg-12 col-md-12 col-xs-12 px-0">   
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            <div>                                                     
                                                                <select wire:model.defer="filiale" class="form-control form-select bordure w-auto"> 
                                                                    <option value=""></option>
                                                                    @foreach($entiteFiliale as $entiteFiliales)
                                                                        <option value="{{$entiteFiliales->id}}">{{$entiteFiliales->enseigne}}</option>
                                                                    @endforeach   
                                                                </select> 
                                                            </div>
                                                            <div>
                                                                <button class="btn btn-black" title="Cliquez pour valider" wire:click.prevent="valider()"><i class="fa fa-check"></i></button> 
                                                            </div>
                                                            <div>
                                                        ...
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="fond_entete_table pointer text-end" wire:click="setOrderField('nombre_users')">Utilisateurs <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table pointer text-end" wire:click="setOrderField('montant_paye')">Montant Payé <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    ...
                                                                    <th class="fond_entete_table">Email</th>   
                                                                ...
                                                                @foreach($liste_entit as $liste_entits) 
                                                                    <tr>                                            
                                                                        @if($liste_entits->logo != null)
                                                                            ...
                                                                            @else
                                                                                <td><a class="text-bleu" href="entite?id={{$liste_entits->id}}&active=12&champ=1-3" wire:navigate><img class="img_produit" src="storage/{{$liste_entits->logo}}"/></a></td>
                                                                            @endif    
                                                                        @else
                                                                            @if(auth()->user()->societe =="Administration") 
                                                                            ...
                                                                            @else
                                                                                <td><a class="text-bleu" href="entite?id={{$liste_entits->id}}&active=12&champ=1-3" wire:navigate><img class="img_produit" src="storage/default/image.png"/></a></td>
                                                                            @endif 
                                                                        @endif    
                                                                        @if(auth()->user()->societe =="Administration")                                                               
                                                                        ...
                                                                        @else 
                                                                            <td class="fw-bold"><a class="text-bleu" href="entite?id={{$liste_entits->id}}&active=12&champ=1-3" wire:navigate><i class="fa fa-university"></i> {{substr($liste_entits->enseigne,0,52) > substr($liste_entits->enseigne,0,51) ? substr($liste_entits->enseigne,0,52).'...': $liste_entits->enseigne}}</a></td>                                                                      
                                                                        @endif
                                                                        ...
                                                                        <td class="fw-semibold text-end">{{$liste_entits->nombre_users}} / {{$liste_entits->nbre_user_max}} <i class="fa fa-users"></i></td> 
                                                                        <td class="fw-bold text-end">{{$liste_entits->montant_paye}}</td>
                                                                        <td class="">
                                                                            @if($liste_entits->periode == "PointVente Annuel")
                                                                                <span class="fw-bold text-vert">{{$liste_entits->periode}}</span>                                           
                                                                            @elseif($liste_entits->periode == "PointVente Mensuel")
                                                                                <span class="fw-bold text-orange">{{$liste_entits->periode}}</span>                                           
                                                                            @elseif($liste_entits->periode == "Pressing Annuel")                                            
                                                                        ...
                                                                        @if(auth()->user()->societe =="Administration")    
                                                                            <td class=""><a href="detail_entite?id={{$liste_entits->id}}" wire:navigate><i class="fa fa-eye btn-outline-muted" title="Cliquez pour modifier" data-toggle="tooltip"></i></a></td>
                                                                        @else
                                                                            <td class=""><a href="entite?id={{$liste_entits->id}}" wire:navigate><i class="fa fa-eye btn-outline-muted" title="Cliquez pour modifier" data-toggle="tooltip"></i></a></td>
                                                                        @endif 
                                                                        @if(auth()->user()->societe =="Administration")
                                                                            <td class="text-end">
                                                                                @if($confirmer === $liste_entits->id)
                                                                                    <a wire:click.prevent="supprimer({{$liste_entits->id}})"><i class="fa fa-trash bg-red text-white w-32 px-1 py-1 rounded border pointer" style="font-size: 8px;" title="Cliquez pour confirmer la suppression" data-toggle="tooltip"> Confirmer?</i></a>
                                                                                @else
                                                                                    <a wire:click.prevent="confirmerDelete({{$liste_entits->id}})" class="btn-outline-muted btn-xs pointer"><i class="fa fa-trash" title="Cliquez pour supprimer" data-toggle="tooltip"></i></a>
                                                                                @endif
                                                                            </td>
                                                                        @endif 
                                                                    </tr>                                        
    ...
    @include('livewire.administration.multi_societe.create_filiale')  
</div>
