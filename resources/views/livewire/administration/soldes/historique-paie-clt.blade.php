                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">                                                
                                                ...
                                                <div class="text-nowrap">
                                                    ...
                                                    <span class="d-block fw-semibold text-dark" style="font-size: 13px;">{{number_format($nbreTotalSoldeClient,0,',',' ')}}</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home" title="Listing {{$title_fils}}"><i class="fas fa-receipt text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$soldeClient_count}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">   
                                            <a href="choix_plan" wire:navigate class="btn btn-sm btn-bleu d-none d-md-block ms-auto"><i class="fa fa-chevron-left"></i> Retour liste</a>
                                        </div>
                                    </div>
                                </div>
                                            ...
                                            <div class="col-lg-12 col-md-12 col-xs-12 px-0">   
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    ...
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="fond_entete_table pointer text-start" wire:click="setOrderField('responsable_societe')">PDG <i class="fa fa-arrow-down-short-wide"></i></th>                                 
                                                            ...
                                                            <tbody>
                                                                @foreach($liste_soldeClient as $liste_soldeClients) 
                                                                    <tr>  
                                                                        <td class="fw-semibold">#{{$liste_soldeClients->id}}</td>  {{--entite--}}                                        
                                                                        @if($liste_soldeClients->logo != null)
                                                                            <td>
                                                                                @if(auth()->user()->societe == "Administration")
                                                                                    <a class="text-bleu" href="detail_entite?id={{$liste_soldeClients->id_enseigne}}&active=12&champ=1-3" wire:navigate><img class="img_produit" src="storage/{{$liste_soldeClients->logo}}"/></a>
                                                                                @else
                                                                                    <a class="text-bleu" href="entite?id={{$liste_soldeClients->id_enseigne}}&active=12&champ=1-3" wire:navigate><img class="img_produit" src="storage/{{$liste_soldeClients->logo}}"/></a>
                                                                                @endif  
                                                                            </td>                                            
                                                                        @else
                                                                            <td>
                                                                                @if(auth()->user()->societe == "Administration")
                                                                                    <a class="text-bleu" href="detail_entite?id={{$liste_soldeClients->id_enseigne}}&active=12&champ=1-3" wire:navigate><img class="img_produit" src="storage/default/image.png"/></a>
                                                                                @else
                                                                                    <a class="text-bleu" href="entite?id={{$liste_soldeClients->id_enseigne}}&active=12&champ=1-3" wire:navigate><img class="img_produit" src="storage/default/image.png"/></a>
                                                                                @endif
                                                                            </td>
                                                                        @endif                                                                   
                                                                        <td class="fw-bold">
                                                                            @if(auth()->user()->societe == "Administration")
                                                                                <a class="text-bleu" href="detail_entite?id={{$liste_soldeClients->id_enseigne}}&active=12&champ=1-3" wire:navigate><i class="fa fa-university"></i> {{substr($liste_soldeClients->enseigne,0,52) > substr($liste_soldeClients->enseigne,0,51) ? substr($liste_soldeClients->enseigne,0,52).'...': $liste_soldeClients->enseigne}}</a>
                                                                            @else
                                                                                <a class="text-bleu" href="entite?id={{$liste_soldeClients->id_enseigne}}&active=12&champ=1-3" wire:navigate><i class="fa fa-university"></i> {{substr($liste_soldeClients->enseigne,0,52) > substr($liste_soldeClients->enseigne,0,51) ? substr($liste_soldeClients->enseigne,0,52).'...': $liste_soldeClients->enseigne}}</a>
                                                                            @endif
                                                                        </td>                                                                      
                                                                        <td class="fw-semibold">{{$liste_soldeClients->societe_mere}}</td> 
                                                                        <td class="fw-semibold">{{$liste_soldeClients->designation}}</td> 
                                                                        <td class="fw-semibold text-end text-danger">{{number_format($liste_soldeClients->debit,0,',',' ')}}</td> 
                                                                        <td class="fw-semibold text-end text-vert">{{number_format($liste_soldeClients->credit,0,',',' ')}}</td>                                           
                                                                        <td class="fw-semibold">{{$liste_soldeClients->responsable_societe}}</td> 
                                                                        <td class="fw-semibold">{{$liste_soldeClients->ville}}</td> 
                                                                        <td class="fw-semibold">{{$liste_soldeClients->pays}}</td> 
                                                                        <td class="">{{$liste_soldeClients->telephone}}</td>
                                                                        <td class="fw-semibold">{{$liste_soldeClients->email}}</td> 
                                                                        <td class="fw-semibold"><a href="detail_user?id={{$liste_soldeClients->user_id}}&active=12&champ=1-1" wire:navigate><i class="fas fa-user-circle text-danger"></i> {{$liste_soldeClients->nom_user}}</a></td> 
                                                                        <td class="">{{date('d-m-Y H:i:s',strtotime($liste_soldeClients->created_at))}}</td>
                                                                    </tr>                                        
                                                                @endforeach
                                                                <tr>                                                                     
                                                        ...
                                                        <div class="bloc_pagination">{{$liste_soldeClient->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
