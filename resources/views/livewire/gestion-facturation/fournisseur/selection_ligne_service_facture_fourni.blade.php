                            ...
                            <div class="table-outer">
                                <div class="table-responsive border rounded-1">  
                                    <table class="table table-striped table-hover text-nowrap m-0">
                                        <thead>
                                            <tr>
                                                <th class="fond_entete_table pointer" wire:click="setOrderField('nom_produit')">Produit (<span class="text-info">{{$service_produitCount}}</span>)</th>   
                                                <th class="fond_entete_table text-end pointer" wire:click="setOrderField('prix_achat')">Prix</i></th>
                                                <th class="fond_entete_table pointer" wire:click="setOrderField('reference')">Référence</th>
                                                ...
                                                <th colspan="2" class="fond_entete_table text-start pointer"></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <tr>                                       
                                                <td class="fw-bold pointer" wire:click="afficheLigneService({{$service_produits->id}})">
                                                    <i class="fa fa-refresh"></i> {{substr($service_produits->nom_produit,0,52) > substr($service_produits->nom_produit,0,51) ? substr($service_produits->nom_produit,0,52).'...': $service_produits->nom_produit}}
                                                </td>  
                                                <td class="text-end fw-semibold pointer" wire:click="afficheLigneService({{$service_produits->id}})">
                                                    <span class="text-bleu fw-bold">{{$service_produits->prix_achat}}</span>
                                                </td>                                                                     
