                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total facture fournisseur</span>
                                            ...
                                            <div class="d-flex align-items-center px-3 flex-fill">
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Quantité</span>
                                                    <span class="d-block fw-bold text-dark mt-1" style="font-size: 14px;">{{number_format($quantite_all,0,',',' ')}}</span>
                                                </div>
                                            </div>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home"><i class="fas fa-file-invoice text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$factFourniCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">  
                                            {{-- <button class="btn btn-sm btn-danger ms-auto" title="Cliquez pour enregistrer" data-toggle="tooltip" wire:click.prevent="()"><i class="fa fa-plus-circle"></i> Nouvelle facture</button>                           --}}
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div>
                                                                ...
                                                                <input type="search" wire:model.live="parFact" id="parRef" class="form-control bordure" placeholder="Rech. réf. facture">
                                                            </div>
                                                            <div class="">
                                                    ...
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            ...
                                                            <tbody>
                                                                @foreach($fact_fourni as $fact_fournis) 
                                                                    <tr>
                                                                        <td class="fw-semibold pointer"><a href="detail_product?id={{$fact_fournis->id_produit}}&active=4&champ=1-1&choix=2" wire:navigate class="text-bleu"><i class="fa fa-cube"></i> {{substr($fact_fournis->produit,0,52) > substr($fact_fournis->produit,0,51) ? substr($fact_fournis->produit,0,52).'...': $fact_fournis->produit}}</a></td>
                                                                        <td class="fw-semibold text-center">{{number_format($fact_fournis->quantite,0,',',' ')}}</td>
                                                                        <td class="fw-semibold pointer"><a href="nouveau_fact_fourni?id={{$fact_fournis->id_facture_fournisseur_entete}}&ref={{$fact_fournis->code_facture}}&active=7&champ=2-1&choix=1" wire:navigate><i class="fas fa-file-invoice-dollar text-danger" aria-hidden="true"></i> {{$fact_fournis->code_facture}}</a></td> 
                                                                        <td class="fw-semibold pointer"><a href="detail_tier?id={{$fact_fournis->id_fournisseur}}&ref={{$fact_fournis->code_facture}}&active=3&champ=3-2" wire:navigate class="text-bleu">@if($fact_fournis->nom_fournisseur) <i class="fa fa-user-circle"></i> @endif {{substr($fact_fournis->nom_fournisseur,0,52) > substr($fact_fournis->nom_fournisseur,0,51) ? substr($fact_fournis->nom_fournisseur,0,52).'...': $fact_fournis->nom_fournisseur}}</a></td>                                                                      
                                                                        <td class="fw-semibold text-end">{{date('d-m-Y', strtotime($fact_fournis->created_at))}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($fact_fournis->montant_ttc,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($fact_fournis->montant_remise,0,',',' ')}}</td>
                                                                        @if($autoriser == 1)
                                                                            <td class="fw-semibold text-end">{{number_format($fact_fournis->marge,0,',',' ')}}</td>
                                                                        @endif
                                                                        <td class="text-end">
                                                                            @if($fact_fournis->etat == "Brouillon")
                                                                                <span class="badge bg-secondary py-1" title="Facture {{$fact_fournis->etat}}"><i class="fa fa-box"></i> {{$fact_fournis->etat}}</span>
                                                                            @elseif($fact_fournis->etat == "Impayée")
                                                                                <span class="badge bg-danger py-1" title="Facture {{$fact_fournis->etat}}"><i class="fa fa-times-circle"></i> {{$fact_fournis->etat}}</span>
                                                                            @elseif($fact_fournis->etat == "Payée")
                                                                                <span class="badge bg-success py-1" title="Facture {{$fact_fournis->etat}}"><i class="fa fa-check-circle"></i> {{$fact_fournis->etat}}</span>
                                                                            @elseif($fact_fournis->etat == "Commencée")
                                                                                <span class="badge bg-info py-1" title="Facture {{$fact_fournis->etat}}"><i class="fa fa-recycle"></i> {{$fact_fournis->etat}}</span>
                                                                            @endif                                                 
                                                                        </td>  
                                                                        <td class="pointer"><a href="detail_user?id={{$fact_fournis->user_id}}&active=12&champ=1-1" wire:navigate><i class="fas fa-user-circle text-bleu" aria-hidden="true"></i> {{$fact_fournis->nom_user}}</a></td>                               
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($fact_fournis->updated_at))}}</td>
                                                                        <td class="text-end"><a href="nouveau_fact_fourni?id={{$fact_fournis->id_facture_fournisseur_entete}}&ref={{$fact_fournis->code_facture}}&active=7&champ=2-1&choix=1" wire:navigate class=""  title="Cliquez pour modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-pencil"></i></a></td>
                                                                        {{-- <td class="taille_icon text-end">
                                                                            @if($confirmer === $fact_fournis->id)                                                               
                                                                                <a wire:click.prevent="supprimer({{$fact_fournis->id}},'{{$fact_fournis->code_facture}}')" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size:8px;"  title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer ?</i></a>
                                                                            @else
                                                                                <a wire:click.prevent="confirmerDelete({{$fact_fournis->id}})" class="btn btn-outline-muted btn-xs"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                            @endif
                                                                        </td>  --}}
                                                                    </tr>
                                                                @endforeach
                                                        ...
                                                        <div class="bloc_pagination">{{$fact_fourni->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
