<div x-data="{selection: @entangle('selection').defer}">
                                    ...
                                    <div class="card-body border border-light-subtle rounded-3 bg-white position-relative custom-scroll pe-2" style="height: calc(100vh - 131px); overflow-y: auto;">
                                        <div class="text-centerk">
                                            ...
                                            <div class="d-flex align-items-center justify-content-end gap-1">
                                                ...
                                                <div class="statusref">
                                                    @if($etat_expedi == "Brouillon")
                                                        <span class="badge bg-secondary py-1" title="Expédition {{$etat_expedi}}"><i class="fas fa-dolly"></i> {{$etat_expedi}}</span>
                                                    @elseif($etat_expedi == "Clôturée")
                                                        <span class="badge bg-danger py-1" title="Expédition {{$etat_expedi}}"><i class="fas fa-dolly"></i> {{$etat_expedi}}</span>
                                                    @elseif($etat_expedi == "Partiel")
                                                        <span class="badge bg-primary py-1" title="Expédition {{$etat_expedi}}"><i class="fas fa-dolly"></i> {{$etat_expedi}}</span> 
                                                    @endif
                                                </div>                            
                                            </div>
                                        </div>
                                        ...
                                        <div class="d-flex gap-1 mb-2">  
                                            <a href="listing_fact_clt?active=7&champ=1-1&choix=1" wire:navigate class="btn btn-sm btn-outline-danger flex-fill" title="Cliquez pour fermer" data-toggle="tooltip"><i class="fa fa-close"></i> Fermer</a>
                                            ...
                                            @if($factClientLigneCount)
                                                ...
                                                @elseif($etat == 'Payée')
                                                    @if(substr($this->reference,0,3) != "POS") 
                                                        <a href="#" wire:click.prevent="expedition()" class="btn btn-sm btn-outline-primary flex-fill fw-semibold ms-auto" title="Cliquez pour créer une expedition" data-toggle="tooltip"><i class="fas fa-dolly"></i> Créer expédition</a>
                                                    @endif  
                                                @endif  
                                            @endif 
                                        </div> 
                                        ...
                                        <div class="table-responsive border-top rounded-0">                                        
                                            <table class="table table-striped table-hover text-nowrap m-0">
                                                ...
                                                <tbody>
                                                    ...
                                                    @endforeach
                                                    @foreach($expCltEntete as $expCltEntetes)
                                                        <tr>
                                                            <td class="fw-semibold"><a href="detail_expedition_clt?id={{$expCltEntetes->id}}&ref={{$expCltEntetes->code_expedition}}&active=6&champ=1-1&choix=2" wire:navigate>Expédition</a></td>                                                        
                                                            <td class="fw-semibold"><a href="detail_expedition_clt?id={{$expCltEntetes->id}}&ref={{$expCltEntetes->code_expedition}}&active=6&champ=1-1&choix=2" wire:navigate class="text-bleu"><i class="fas fa-dolly text-danger" aria-hidden="true"></i> {{$expCltEntetes->code_expedition}}</a></td>                                                        
                                                            <td class="fw-semibold text-end">{{number_format($expCltEntetes->montant_ttc,0,',',' ')}} {{$this->devise}}</td>
                                                            <td class="fw-semibold"> 
                                                                @if($expCltEntetes->etat == "Clôturée")
                                                                    <span class="badge bg-danger" title="Commande {{$expCltEntetes->code_commande}}"><i class="fa fa-check-circle"></i> {{$expCltEntetes->etat}}</span>                                                        
                                                                @elseif($expCltEntetes->etat == "Partiel")
                                                                    <span class="badge bg-primary" title="Commande {{$expCltEntetes->code_commande}}"><i class="fa fa-recycle"></i> {{$expCltEntetes->etat}}</span>  
                                                                @endif                                                              
                                                            </td>
                                                            <td class="fw-semibold"><i class="fas fa-calendar-alt text-danger"></i> {{$expCltEntetes->created_at ? date('d-m-Y H:i:s', strtotime($expCltEntetes->created_at)): "" }}</td>                                                        
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                            </table>  
                            ...
                            <div class="col-lg-9">
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-0">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                            <a href="listing_fact_clt?active=7&champ=1-1&choix=1" wire:navigate class="btn btn-sm btn-bleu d-none d-md-block ms-auto"><i class="fa fa-chevron-left"></i> Retour liste</a>
                                        ...
                                        <div class="card shadow-sm">  
                                            <div class="card shadow-sm border border-light-subtle rounded-3 bg-white p-2">    
                                                    ...
                                                    <div class="hauteur_ecran">
                                                        <div class="row pt-1 px-2 pb-2">
                                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                                                                {{-- <label for="nom_opportunite" class="col-md-5 col-sm-3 fw-semibold col-form-label">Facture client</label> --}}
                                                                <div class="row mb-1">
                                                            ...
                                                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">                                            
                                                                <div class="row mb-1">
                                                                    <label for="reference" class="col-lg-3 col-md-3 col-sm-4 fw-bold col-form-label">Référence</label>
                                                                ...
                                                                <div class="row mb-1">
                                                                    <label for="date_facturation" class="col-lg-5 col-md-5 col-sm-4 fw-bold col-form-label">Date de facturation</label>
                                                                    <div class="col-lg-7 col-md-7 col-sm-8">
                                                                        <input type="date" wire:model="date_facturation" class="form-control bordure w-75 @error('date_facturation') is-invalid @enderror" id="date_facturation">
                                                                    </div>
                                                                    <div class="d-flex justify-content-start">
                                                                ...
                                                                <div class="row mb-1">
                                                                    <label for="date_echeance" class="col-lg-5 col-md-5 col-sm-4 fw-bold col-form-label">Date échéance</label>
                                                                    <div class="col-lg-7 col-md-7 col-sm-8">
                                                                        <input type="date" wire:model="date_echeance"  class="form-control bordure w-75 @error('date_echeance') is-invalid @enderror" id="date_echeance">
                                                                    </div>
                                                                    <div class="d-flex justify-content-start">
                                                            ...
                                                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                                                                ...
                                                                <div class="row mb-1">
                                                                    <label for="compte_bancaire" class="col-lg-5 col-md-5 col-sm-4 fw-bold col-form-label">Compte bancaire</label>
                                                                    <div class="col-lg-7 col-md-7 col-sm-8">                                                        
                                                            ...
                                                            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 px-0">
                                                                <div class="table-responsive mt-2"> 
                                                                    <ul class="nav nav-tabs border-0" role="tablist">
                                                                        <li class="nav-item">
                                                                            <a class="nav-link active" data-bs-toggle="tab" href="#ligneFact">Ligne de facture (<span class="text-vert">{{$factClientLigneCount}}</span>)</a>
                                                                        </li>
                                                                    </ul>
                                                                    <div class="tab-content table-responsive px-0" style="border-top-style: dashed; border-top-width: 1px;border-top-color: #0e04043d;">
                                                                        <div id="ligneFact" class="tab-pane active">                               
                                                                            <table class="table table-striped text-nowrap m-0"> 
                                                                                ...
                                                                                <tbody>
                                                                                    @foreach($factClient_ligne as $factClient_lignes)
                                                                                        <tr> 
                                                                                            <td class="fw-bold"><a href="detail_product?id={{$factClient_lignes->id_produit}}&active=4&champ=1-1&choix=2" wire:navigate class="text-bleu fw-bold">{{Str::limit($factClient_lignes->produit, 32)}} - <span class="text-muted">{{Str::limit($factClient_lignes->reference, 22)}}</a></td>                                                         
                                                                                            <td class="text-muted fw-semibold">{{$factClient_lignes->type_produit}}</td>  
                                                                                            <td class="fw-bold text-end">{{$factClient_lignes->prix_vente}}</td>     
                                                                                            <td class="text-center fw-bold"> 
                                                                                                @if($factClient_lignes->quantite_expediee == 0)
                                                                                                    {{$factClient_lignes->quantite}}
                                                                                                @else 
                                                                                                    {{$factClient_lignes->quantite_expediee}}
                                                                                                @endif
                                                                                            </td>         
                                                                                            <td class="text-center fw-semibold @if($factClient_lignes->remise > 0) text-vert @endif">{{$factClient_lignes->remise}}%</td>         
                                                                                            <td class="text-end fw-semibold">{{$factClient_lignes->tva}}%</td>         
                                                                                            <td class="text-center fw-semibold">{{$factClient_lignes->precompte}}%</td> 
                                                                                            <td class="text-center fw-semibold @if($factClient_lignes->offrir == "Oui") text-danger @endif">{{$factClient_lignes->offrir}}</td> 
                                                                                            <td class="text-end fw-bold">{{number_format($factClient_lignes->montant_ttc,0,',',' ')}}</td>
                                                                                            <td class="text-end fw-semibold">
                                                                                            @if($etat == 'Brouillon')
                                                                                                @if($confirmer === $factClient_lignes->id)                                                               
                                                                                                    <a wire:click.prevent="supprimer({{$factClient_lignes->id}})" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size: 8px;" title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer ?</i></a>
                                                                                                @else
                                                                                                    <a wire:click.prevent="confirmerDelete({{$factClient_lignes->id}})" class="btn btn-outline-muted btn-xs"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                                                @endif
                                                                                            @endif
                                                                                            </td> 
                                                                                        </tr> 
                                                                                        ...
                                                                                        <tr style="font-size: 12px;">
                                                                                            <td colspan="10">
                                                                                                @if($etat_expedi != 'Clôturée')
                                                                                                    <a href="#" class="btn btn-sm btn-white text-green-100" wire:click.prevent="ajoutLigne(1)" title="Cliquez pour ajouter un produit ou service" data-toggle="tooltip"> <i class="fa fa-plus"></i> Ajouter un Produit / Service</a>
                                                                                                    @if($ouvre === 1)
                                                                        ...
                                                                        <div class="row">
                                                                            ...
                                                                            <div class="col-lg-6 col-md-8 col-sm-6">                                          
                                                                                <table class="table text-nowrap m-0">                                        
                                                                                    <tbody>
                                                                                            <tr>                                                                     
                                                                                                <td class="titlefield text-muted fw-semibold" style="font-size: 13px">Montant HT @if($montantRemise) (Remise : {{number_format($montantRemise,0,',',' ')}} <span class="text-bleu" style="font-size: 11px">{{$this->devise}}</span>) @endif</td>                                                         
                                                                                    ...
                                                                                    <table class="table table-striped table-hover text-nowrap m-0"> 
                                                                                        ...
                                                                                        <tbody> 
                                                                                            @foreach($reglementClient as $reglementClients)
                                                                                                <tr> 
                                                                                                    <td class="fw-semibold">{{$reglementClients->ref_reglement}}</td> 
                                                                                                    <td>{{date('d-m-Y H:i:s', strtotime($reglementClients->created_at))}}</td> 
                                                                                                    <td>{{$reglementClients->mode_reglement}}</td> 
                                                                                                    <td><a href="detail_banque?id={{$reglementClients->id_compte_bancaire}}&active=8&champ=1-1" wire:navigate class="text-bleu fw-semibold">{{$reglementClients->compte_bancaire}}</a></td>
                                                                                                    <td class="text-end text-vert fw-semibold">{{number_format($reglementClients->montant_regler,0,',',' ')}}</td> 
                                                                                                    <td class="taille_icon text-end">
                                                                                                        @if($confirmation === $reglementClients->id)                                                               
                                                                                                            <a wire:click.prevent="effacer({{$reglementClients->id}},{{$reglementClients->id_compte_bancaire}})" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size:8px;" title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> ?</i></a>
                                                                                                        @else
                                                                                                            <a wire:click.prevent="confirmationDelete({{$reglementClients->id}})" class="btn btn-outline-muted btn-xs" style="font-size:9px;" title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                                                        @endif
                                                                                                    </td> 
                                                                                                </tr> 
                                                                                            @endforeach
                                                                                            <tr> 
                                                                                                ...
                                                                                                <td class="text-end fw-bold text-bleu">{{number_format($dejaRegler,0,',',' ')}} <span class="text-bleu" style="font-size: 11px">{{$this->devise}}</span></td> 
                                                                                                {{-- <td class="text-end"></td>  --}}
                                                                                            </tr> 
                                                                                            <tr> 
                                                                                                <td colspan="5" class="text-end fw-bold">Facturé</td> 
                                                                                                <td class="text-end fw-bold">{{number_format($montantTTC,0,',',' ')}} <span class="text-bleu" style="font-size: 11px">{{$this->devise}}</span></td> 
                                                                                                {{-- <td class="text-end"></td>  --}}
                                                                                            </tr>
                                                                                            <tr> 
                                                                                                <td colspan="5" class="text-end fw-bold">Reste à payer</td> 
                                                                                                <td class="text-end text-danger fw-bold fs-5">{{number_format($reste_a_percevoir,0,',',' ')}} <span class="text-bleu" style="font-size: 11px">{{$this->devise}}</span></td> 
                                                                                                {{-- <td class="text-end"></td>  --}}
                                                                                            </tr>
                                                                                        </tbody>
                                                                                    </table>
                                                ...
                                                <div class="alert mt-3 mb-0 d-flex justify-content-between align-items-center rounded-2 px-3 py-2" style="background-color: #f4f8ff; border: 1px solid #cce0ff;">
                                                    ...
                                                    <a href="{{asset('listing_fact_clt?active=7&champ=1-1&choix=1')}}" wire:navigate class="text-decoration-none fw-semibold text-primary" style="font-size: 13px;">Voir plus</a>
                                                </div>
                                            </div>                                                
                                        </div>                                            
                                    </div>                                     
    ...
    {{-- Modal --}}
   @include('livewire.gestion-facturation.client.saisie_paiement') 
