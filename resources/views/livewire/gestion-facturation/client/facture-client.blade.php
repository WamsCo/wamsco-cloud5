                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total facture client</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home"><i class="fas fa-file-invoice text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$factClientCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1 mb-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <a href="{{asset('exporter-fact_client?date_debut='.$this->date_debut.'&date_fin='.$this->date_fin.'&statut='.$this->parEtat.'&client='.$this->query.'&ref='.$this->parRef.'&user='.$this->parUser)}}" class="btn btn-sm btn-success mt-2 d-none d-md-block mb-1"><i class="fa-solid fa-file-export"></i> Exporter Excel</a>
                                                    ...
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            ...
                                                            <tbody>
                                                                @foreach($fact_client as $fact_clients) 
                                                                    <tr>
                                                                        <td class="fw-semibold pointer"><a href="nouveau_fact_clt?id={{$fact_clients->id}}&ref={{$fact_clients->code_facture}}&active=7&champ=1-1&choix=1" wire:navigate><i class="fas fa-file-invoice-dollar text-danger" aria-hidden="true"></i> {{$fact_clients->code_facture}}</a></td> 
                                                                        <td class="fw-semibold pointer"><a href="detail_tier?id={{$fact_clients->id_client}}&ref={{$fact_clients->code_facture}}&active=3&champ=3-2" wire:navigate class="text-bleu">@if($fact_clients->nom_client) <i class="fa fa-user-circle"></i> @endif {{Str::limit($fact_clients->nom_client, 52)}}</a></td>                                                                      
                                                                        <td class="fw-semibold text-start">{{$fact_clients->telephone}}</td>
                                                                        <td class="fw-semibold"><a href="detail_pos_session?id={{$fact_clients->id_session_pos}}&active=5&champ=1-1" wire:navigate class="">@if($fact_clients->ref_session_pos) <i class="fa fa-cash-register text-vert" aria-hidden="true"></i>@endif {{$fact_clients->ref_session_pos}}</a></td>   
                                                                        <td class="">{{$fact_clients->mode_reglement}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($fact_clients->montant_ttc,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($fact_clients->montant_recu,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($fact_clients->reste_a_percevoir,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($fact_clients->montant_remise,0,',',' ')}}</td>
                                                                        @if($autoriser == 1)
                                                                            <td class="fw-semibold text-end">{{number_format($fact_clients->marge,0,',',' ')}}</td>
                                                                        @endif
                                                                        <td class="fw-semibold"><a href="detail_banque?id={{$fact_clients->id_compte_bancaire}}" wire:navigate class="text-bleu">@if($fact_clients->compte_bancaire)<i class="fa fa-university text-danger" aria-hidden="true"></i> @endif {{$fact_clients->compte_bancaire}}</a></td>                                               
                                                                        <td class="text-start">
                                                                            @if($fact_clients->etat == "Brouillon")
                                                                                <span class="badge bg-secondary" title="Facture {{$fact_clients->etat}}"><i class="fa fa-box"></i> {{$fact_clients->etat}}</span>
                                                                            @elseif($fact_clients->etat == "Impayée")
                                                                                <span class="badge bg-warning" title="Facture {{$fact_clients->etat}}"><i class="fa fa-times-circle"></i> {{$fact_clients->etat}}</span>
                                                                            @elseif($fact_clients->etat == "Payée")
                                                                                <span class="badge bg-success" title="Facture {{$fact_clients->etat}}"><i class="fa fa-check-circle"></i> {{$fact_clients->etat}}</span>
                                                                            @elseif($fact_clients->etat == "Commencée")
                                                                                <span class="badge bg-info" title="Facture {{$fact_clients->etat}}"><i class="fa fa-recycle"></i> {{$fact_clients->etat}}</span>
                                                                            @endif                                                 
                                                                        </td>  
                                                                        <td class="text-start">
                                                                            @if($fact_clients->etat_expedi == "Brouillon")
                                                                            <span class="badge bg-secondary" title="Expédition {{$fact_clients->etat_expedi}}"><i class="fas fa-dolly"></i> {{$fact_clients->etat_expedi}}</span>
                                                                            @elseif($fact_clients->etat_expedi == "Clôturée")
                                                                                <span class="badge bg-danger" title="Expédition {{$fact_clients->etat_expedi}}"><i class="fas fa-dolly"></i> {{$fact_clients->etat_expedi}}</span>
                                                                            @elseif($fact_clients->etat_expedi == "Partiel")
                                                                                <span class="badge bg-primary" title="Expédition {{$fact_clients->etat_expedi}}"><i class="fas fa-dolly"></i> {{$fact_clients->etat_expedi}}</span>
                                                                            @elseif($fact_clients->etat_expedi == "")
                                                                                <span class="badge bg-light text-dark" title="Expédition non créée"><i class="fas fa-dolly"></i> Non créée</span>
                                                                            @endif   
                                                                        </td> 
                                                                        <td class="">{{date('d-m-Y', strtotime($fact_clients->date_facturation))}}</td>
                                                                        <td class="">{{date('d-m-Y', strtotime($fact_clients->date_echeance))}}</td>
                                                                        <td class="fw-semibold text-muted pointer"><a href="detail_user?id={{$fact_clients->user_id}}&active=12&champ=1-1" wire:navigate><i class="fas fa-user-circle text-bleu" aria-hidden="true"></i> {{$fact_clients->nom_user}}</a></td>                                            
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($fact_clients->updated_at))}}</td>
                                                                        <td class="text-end"><a href="nouveau_fact_clt?id={{$fact_clients->id}}&ref={{$fact_clients->code_facture}}" wire:navigate class="btn-outline-muted"  title="Cliquez pour voir ou modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-eye"></i></a></td>
                                                                        {{-- <td class="taille_icon text-end">
                                                                            @if($confirmer === $fact_clients->id)                                                               
                                                                                <a wire:click.prevent="supprimer({{$fact_clients->id}},'{{$fact_clients->code_facture}}')" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size:8px;"  title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer ?</i></a>
                                                                            @else
                                                                                <a wire:click.prevent="confirmerDelete({{$fact_clients->id}})" class="btn btn-outline-muted btn-xs"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                            @endif
                                                                        </td>  --}}
                                                                    </tr>
                                                                @endforeach
                                                        ...
                                                        <div class="bloc_pagination">{{$fact_client->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
