                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <h6><a href="{{asset('exporter-fact_client')}}" class="btn btn-sm btn-success mt-2 d-none d-md-block"><i class="fa-solid fa-file-export"></i> Exporter Excel</a></h6>
                                                        ...
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            ...
                                                            <tbody>
                                                                @foreach($fact_client as $fact_clients) 
                                                                    <tr>
                                                                        <td class="fw-semibold pointer"><a href="detail_product?id={{$fact_clients->id_produit}}&active=4&champ=1-1&choix=2" wire:navigate class="text-bleu"><i class="fa fa-cube"></i> {{substr($fact_clients->produit,0,52) > substr($fact_clients->produit,0,51) ? substr($fact_clients->produit,0,52).'...': $fact_clients->produit}}</a></td>
                                                                        <td class="fw-semibold text-center">{{number_format($fact_clients->quantite,0,',',' ')}}</td>
                                                                        <td class="fw-semibold pointer"><a href="nouveau_fact_clt?id={{$fact_clients->id_facture_client_entete}}&ref={{$fact_clients->code_facture}}&active=7&champ=1-1&choix=1" wire:navigate><i class="fas fa-file-invoice-dollar text-danger" aria-hidden="true"></i> {{$fact_clients->code_facture}}</a></td> 
                                                                        <td class="fw-semibold pointer"><a href="detail_tier?id={{$fact_clients->id_client}}&ref={{$fact_clients->code_facture}}&active=3&champ=3-2" wire:navigate class="text-bleu">@if($fact_clients->nom_client) <i class="fa fa-user-circle"></i> @endif {{substr($fact_clients->nom_client,0,52) > substr($fact_clients->nom_client,0,51) ? substr($fact_clients->nom_client,0,52).'...': $fact_clients->nom_client}}</a></td>                                                                      
                                                                        <td class="fw-semibold text-end">{{date('d-m-Y', strtotime($fact_clients->created_at))}}</td>
                                                                        ...
                                                                        <td class="text-end">
                                                                            ...
                                                                            @elseif($fact_clients->etat == "Impayée")
                                                                                <span class="badge bg-danger" title="Facture {{$fact_clients->etat}}"><i class="fa fa-times-circle"></i> {{$fact_clients->etat}}</span>
                                                                        ...
                                                                        <td class="pointer"><a href="detail_user?id={{$fact_clients->user_id}}&active=12&champ=1-1" wire:navigate><i class="fas fa-user-circle text-bleu" aria-hidden="true"></i> {{$fact_clients->nom_user}}</a></td>                                                                            
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($fact_clients->updated_at))}}</td>
                                                                        {{-- <td class="text-end"><a href="nouveau_fact_clt?id={{$fact_clients->id_facture_client_entete}}&ref={{$fact_clients->code_facture}}&active=7&champ=1-1&choix=1" wire:navigate class=""  title="Cliquez pour modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-pencil"></i></a></td> --}}
                                                                    </tr>
                                                                @endforeach
                                                            </tbody>
                                                            <tr>
