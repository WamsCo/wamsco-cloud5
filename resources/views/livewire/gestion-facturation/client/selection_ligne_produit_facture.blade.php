<div>
	...
	{{-- <div wire:ignore.self class="modal fade" id="SelectClientModal" tabindex="-1" role="dialog" aria-labelledby="slidemodalLabel" aria-hidden="true"> --}}
		 {{-- Debut chargement --}}        
			...
			<div class="modal-dialog modal-xl">
				<div class="modal-content">             
					<div class="modal-header py-2" style="background-color: #56585d; color:#ffffff;">
						<h3 class="modal-title fs-5"><i class="fa fa-cube"></i> {{$this->type_produit}} ajouté (<span class="text-warning">{{$factClientLigneCount}}</span>)</h3>
					</div>
                            ...
                            <div class="table-outer">
                                <div class="table-responsive border rounded-1">  
                                    <table class="table table-striped table-hover text-nowrap m-0">
                                        <thead>
                                            <tr>
                                                <th class="fond_entete_table">Produit (<span class="text-info">{{$produit_stockCount}}</span>)</th>   
                                                <th class="fond_entete_table text-end">Valeur à la vente</th>
                                                <th class="fond_entete_table text-end">Quantité</i></th>
                                                ...
                                                <th class="fond_entete_table text-start"></th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <tr>                                       
                                                <td class="fw-bold pointer" wire:click="afficheLigne({{$produit_stocks->id}})">
                                                    <i class="fa fa-cube"></i> {{Str::limit($produit_stocks->nom_produit, 32)}} - <span class="text-muted">{{Str::limit($produit_stocks->reference, 22)}}</span>
                                                </td>    
                                                <td class="text-end fw-semibold pointer" wire:click="afficheLigne({{$produit_stocks->id}})">{{number_format($produit_stocks->valeurVentetotal,2,',',' ')}}</td>
                                                ...
                                                <td class="text-end fw-semibold pointer" wire:click="afficheLigne({{$produit_stocks->id}})">{{number_format($produit_stocks->valorisationAchatTotal,2,',',' ')}}</td>                                                                 
