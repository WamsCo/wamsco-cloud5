                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">                                                
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total règlement client</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home" title="Liste des tieres : Prospects, Clients, Fournisseurs"><i class="fas fa-users text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$regleClientCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <h6><a href="{{asset('exporter-regle_client')}}" class="btn btn-sm btn-success mt-2 d-none d-md-block"><i class="fa-solid fa-file-export"></i> Exporter Excel</a></h6>
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div>
                                                                <label for="parRef" class="sr-only">Recherche</label>
                                                                <input type="search" wire:model.live="parFact" id="parRef" class="form-control bordure" placeholder="Rechercher facture">
                                                            </div>                            
                                                            <div>
                                                    ...
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            ...
                                                            <tbody>
                                                                @foreach($Reglement_client as $Reglement_clients) 
                                                                    <tr>
                                                                        <td class="fw-semibold pointer"><a href="#" wire:navigate><i class="fas fa-file-invoice-dollar text-danger" aria-hidden="true"></i> {{$Reglement_clients->ref_reglement}}</a></td> 
                                                                        <td class="">{{date('d-m-Y', strtotime($Reglement_clients->date_reglement))}}</td>
                                                                        <td class="fw-semibold pointer"><a href="detail_tier?id={{$Reglement_clients->id_client}}&ref={{$Reglement_clients->code_facture}}&active=3&champ=3-2" wire:navigate class="text-bleu">@if($Reglement_clients->nom_client) <i class="fa fa-user-circle"></i> @endif {{substr($Reglement_clients->nom_client,0,52) > substr($Reglement_clients->nom_client,0,51) ? substr($Reglement_clients->nom_client,0,52).'...': $Reglement_clients->nom_client}}</a></td>                                                                      
                                                                        <td class="">{{$Reglement_clients->mode_reglement}}</td>
                                                                        <td class="fw-semibold"><a href="detail_banque?id={{$Reglement_clients->id_compte_bancaire}}"  wire:navigate class="text-bleu">@if($Reglement_clients->compte_bancaire)<i class="fa fa-university text-danger" aria-hidden="true"></i> @endif {{$Reglement_clients->compte_bancaire}}</a></td>                                               
                                                                        <td class="fw-semibold"><a href="#" wire:navigate class="text-bleu"><i class="fa fa-receipt text-danger" aria-hidden="true"></i> {{$Reglement_clients->ecriture_bancaire}}</a></td>                                               
                                                                        <td class="fw-semibold"><a href="nouveau_fact_clt?id={{$Reglement_clients->id_facture_client_entete}}&active=7&champ=1-1&choix=1" wire:navigate class="text-bleu"><i class="fas fa-file-invoice-dollar text-vert" aria-hidden="true"></i> {{$Reglement_clients->code_facture}}</a></td>                                               
                                                                        <td class="fw-semibold"><a href="detail_pos_session?id={{$Reglement_clients->id_session_pos}}&active=5&champ=1-1" wire:navigate class="text-bleu">@if($Reglement_clients->ref_session_pos) <i class="fa fa-cash-register" aria-hidden="true"></i>@endif {{$Reglement_clients->ref_session_pos}}</a></td>                                               
                                                                        <td class="fw-bold text-end">{{number_format($Reglement_clients->montant_regler,0,',',' ')}}</td>
                                                                        <td class="text-start">{{$Reglement_clients->num_cheq_virement}}</td>
                                                                        <td class="text-start">{{$Reglement_clients->commentaire}}</td>  
                                                                        <td class="pointer"><a href="detail_user?id={{$Reglement_clients->user_id}}&active=12&champ=1-1" wire:navigate><i class="fas fa-user-circle text-bleu" aria-hidden="true"></i> {{$Reglement_clients->nom_user}}</a></td>                                    
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($Reglement_clients->updated_at))}}</td>
                                                                        {{-- <td class="text-end"><a href="nouveau_fact_clt?id={{$Reglement_clients->id_facture_client_entete}}" wire:navigate class=""  title="Cliquez pour modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-pencil"></i></a></td> --}}
                                                                    </tr>
                                                                @endforeach
                                                            </tbody>
                                                            <tr>
                                                        ...
                                                        <div class="bloc_pagination">{{$Reglement_client->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
