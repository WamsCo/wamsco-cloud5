<div x-data="{selection: @entangle('selection').defer}" wire:poll.visible.20s>
                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total commande</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home"><i class="fas fa-file-invoice text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$cmdClientCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1 mb-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <a href="{{asset('exporter-cmd_client?date_debut='.$this->date_debut.'&date_fin='.$this->date_fin.'&statut='.$this->parEtat.'&client='.$this->query.'&ref='.$this->parCmd)}}" class="btn btn-sm btn-success mt-2 d-none d-md-block mb-1"><i class="fa-solid fa-file-export"></i> Exporter Excel</a>
                                                    ...
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            ...
                                                            <tbody>
                                                                @foreach($cmd_client as $cmd_clients) 
                                                                    <tr>
                                                                        <td class="fw-semibold pointer"><a href="nouveau_cmd_clt?id={{$cmd_clients->id}}&ref={{$cmd_clients->code_commande}}&active=6&champ=1-1&choix=2" wire:navigate><i class="fas fa-file-invoice text-danger" aria-hidden="true"></i> {{$cmd_clients->code_commande}}</a></td> 
                                                                        <td class="fw-semibold pointer"><a href="detail_tier?id={{$cmd_clients->id_client}}&ref={{$cmd_clients->code_commande}}&active=3&champ=3-2" wire:navigate class="text-bleu">@if($cmd_clients->nom_client) <i class="fa fa-user-circle"></i> @endif {{Str::limit($cmd_clients->nom_client, 52)}}</a></td>                                                                      
                                                                        <td class="fw-semibold text-start">{{$cmd_clients->telephone}}</td>
                                                                        ...
                                                                        <td class="text-start">
                                                                            @if($cmd_clients->etat == "Brouillon")
                                                                                <span class="badge bg-secondary" title="Commande {{$cmd_clients->etat}}"><i class="fa fa-box"></i> {{$cmd_clients->etat}}</span>
                                                                            @elseif($cmd_clients->etat == "Impayée")
                                                                                <span class="badge bg-danger" title="Commande {{$cmd_clients->etat}}"><i class="fa fa-times-circle"></i> {{$cmd_clients->etat}}</span>
                                                                            @elseif($cmd_clients->etat == "Payée")
                                                                                <span class="badge bg-success" title="Commande {{$cmd_clients->etat}}"><i class="fa fa-check-circle"></i> {{$cmd_clients->etat}}</span>
                                                                            @elseif($cmd_clients->etat == "Validée")
                                                                                <span class="badge bg-success" title="Commande {{$cmd_clients->etat}}"><i class="fa fa-check"></i> {{$cmd_clients->etat}}</span>
                                                                            @endif                                                 
                                                                        </td>  
                                                                        ...
                                                                        <td class="text-start">
                                                                            ...
                                                                            <span class="badge bg-secondary" title="Expédition {{$cmd_clients->etat_expedi}}"><i class="fas fa-dolly"></i> {{$cmd_clients->etat_expedi}}</span>
                                                                            @elseif($cmd_clients->etat_expedi == "Clôturée")
                                                                                <span class="badge bg-danger" title="Expédition {{$cmd_clients->etat_expedi}}"><i class="fas fa-dolly"></i> {{$cmd_clients->etat_expedi}}</span>
                                                                            @elseif($cmd_clients->etat_expedi == "Partiel")
                                                                                <span class="badge bg-primary" title="Expédition {{$cmd_clients->etat_expedi}}"><i class="fas fa-dolly"></i> {{$cmd_clients->etat_expedi}}</span>
                                                                        ...
                                                                        <td class="pointer"><a href="detail_user?id={{$cmd_clients->user_id}}&active=12&champ=1-1" wire:navigate><i class="fas fa-user-circle text-bleu" aria-hidden="true"></i> {{$cmd_clients->nom_user}}</a></td>                                                                     
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($cmd_clients->updated_at))}}</td>
                                                                        <td class="text-end"><a href="nouveau_cmd_clt?id={{$cmd_clients->id}}&ref={{$cmd_clients->code_commande}}&active=6&champ=1-1&choix=2" wire:navigate class="btn-outline-muted"  title="Cliquez pour voir ou modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-eye"></i></a></td>
                                                                        {{-- <td class="taille_icon text-end">
                                                                            @if($confirmer === $cmd_clients->id)                                                               
                                                                                <a wire:click.prevent="supprimer({{$cmd_clients->id}},'{{$cmd_clients->code_commande}}')" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size:8px;"  title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer ?</i></a>
                                                                            @else
                                                                                <a wire:click.prevent="confirmerDelete({{$cmd_clients->id}})" class="btn btn-outline-muted btn-xs"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                            @endif
                                                                        </td>  --}}
                                                                    </tr>
                                                                @endforeach 
                                                        ...
                                                        <div class="bloc_pagination">{{$cmd_client->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
