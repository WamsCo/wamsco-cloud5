<div>
    <div wire:ignore.self class="modal fade" id="SelectLigneExpeditionModal" tabindex="-1" data-bs-backdrop="static" data-keyboard="false" role="dialog" aria-labelledby="exampleModalFullscreenLabel" aria-hidden="true">
        <form enctype="multipart/form-data">							
			<div class="modal-dialog modal-lg">
				<div class="modal-content">             
					<div class="modal-header py-2" style="background-color: #56585d; color:#ffffff;">
						<h3 class="modal-title fs-5"><i class="fa fa-dolly"></i> Expédition (<span class="text-warning">{{$expeClientLigneCount}}</span>) » Facturation</h3>
					</div>
                                    ...
                                    <table class="table table-striped table-hover text-nowrap m-0">
                                        ...
                                        <tbody>
                                            @foreach($expeClient_ligne as $expeClient_lignes)                                                    
                                                <tr>
                                                    <td class="text-bleu fw-bold pointer" wire:click="afficheLigneExpedition({{$expeClient_lignes->id}})">
                                                        <i class="fa fa-cube"></i> {{Str::limit($expeClient_lignes->produit, 32)}} - <span class="text-muted">{{Str::limit($expeClient_lignes->reference, 22)}}</span>
                                                    </td>
                                                    <td class="text-muted fw-semibold pointer" wire:click="afficheLigneExpedition({{$expeClient_lignes->id}})">{{$expeClient_lignes->type_produit}}</td>         
                                                    <td class="text-center fw-bold pointer" wire:click="afficheLigneExpedition({{$expeClient_lignes->id}})">{{$expeClient_lignes->quantite}}</td>         
                                                ...
                                                @if($open === $expeClient_lignes->id) 
                                                    <tr>                                                         
                                                        @include('livewire.gestion-commande.client.ajout_ligne_expedition')     
                                                    </tr>
                                                @endif
                                            @endforeach                                            
                                        </tbody>
                                    </table>
                                    <div class="bloc_pagination">{{$expeClient_ligne->links()}}</div> 
                                </div>                        
                            </div>
                        </div>
					</div>
                    <div class="d-flex align-items-center justify-content-betweenk modal-footer py-1" style="background-color: #56585d; color:#ffffff;">
                        {{-- @if($this->verifi == 0)
                            <a href="#" wire:click.prevent="clotureExpedition" target="_blank" class="btn btn-sm btn-secondary" title="Cliquez pour clôturer l'expédition"><i class="fa fa-dolly"></i> Clôturer l'expédition</a>
                        @endif --}}
                        @if($this->client_id)
