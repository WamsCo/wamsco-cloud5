                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total expédition </span>
                                                    <span class="d-block fw-semibold text-dark" style="font-size: 13px;">{{$nbreTotalExpedi}}</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home"><i class="fas fa-file-invoice text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$expediClientCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <a href="{{asset('exporter-exp_client?date_debut='.$this->date_debut.'&date_fin='.$this->date_fin.'&client='.$this->query.'&statut='.$this->parEtat.'&ref='.$this->ParRef)}}" class="btn btn-sm btn-success mt-2 d-none d-md-block mb-1"><i class="fa-solid fa-file-export"></i> Exporter Excel</a>
                                                    ...
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('code_expedition')">Référence <i class="fa fa-arrow-down-short-wide"></i></th>   
                                                                    ...
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('code_facture')">Commande <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                            ...
                                                            <tbody>
                                                                @foreach($expedi_client as $expedi_clients) 
                                                                    <tr>
                                                                        <td class="fw-semibold pointer"><a href="detail_expedition_clt?id={{$expedi_clients->id}}&ref={{$expedi_clients->code_expedition}}&active=6&champ=1-1&choix=3" wire:navigate><i class="fas fa-dolly text-danger" aria-hidden="true"></i> {{$expedi_clients->code_expedition}}</a></td> 
                                                                        <td class="fw-semibold pointer"><a href="detail_tier?id={{$expedi_clients->id_client}}&ref={{$expedi_clients->code_facture}}&active=3&champ=3-2" wire:navigate class="text-bleu">@if($expedi_clients->nom_client) <i class="fa fa-user-circle"></i> @endif {{Str::limit($expedi_clients->nom_client, 52)}}</a></td>                                                                      
                                                                        <td class="">{{date('d-m-Y', strtotime($expedi_clients->created_at))}}</td>
                                                                        <td class="">{{date('d-m-Y', strtotime($expedi_clients->date_echeance))}}</td>
                                                                        <td class="fw-semibold pointer">
                                                                            <a href="#" wire:click.prevent="detailCmd({{$expedi_clients->id_commande_client_entete}}, '{{$expedi_clients->code_commande}}')" class="fw-semibold fs-6" title="Numéro commande client (cliquable)">@if($expedi_clients->code_commande) <i class="fas fa-file-invoice text-info"></i> @endif {{$expedi_clients->code_commande}}</a>
                                                                        </td> 
                                                                        <td class="fw-semibold pointer">
                                                                            <a href="#" wire:click.prevent="detailFact({{$expedi_clients->id_facture_client_entete}}, '{{$expedi_clients->code_facture}}')" class="fw-semibold fs-6" title="Numéro facture client (cliquable)"> @if($expedi_clients->code_facture) <i class="fas fa-file-invoice-dollar text-vert"></i> @endif {{$expedi_clients->code_facture}}</a>
                                                                        </td> 
                                                                        <td class="fw-semibold text-end">{{number_format($expedi_clients->montant_ttc,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($expedi_clients->montant_recu,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-start">{{$expedi_clients->methode_expedition}}</td>
                                                                        <td class="text-start">
                                                                            @if($expedi_clients->etat == "Brouillon")
                                                                            <span class="badge bg-secondary" title="Expédition {{$expedi_clients->etat}}"><i class="fa fa-box"></i> {{$expedi_clients->etat}}</span>
                                                                            @elseif($expedi_clients->etat == "Clôturée")
                                                                                <span class="badge bg-danger" title="Expédition {{$expedi_clients->etat}}"><i class="fa fa-thumbs-up"></i> {{$expedi_clients->etat}}</span>
                                                                            @elseif($expedi_clients->etat == "Partiel")
                                                                                <span class="badge bg-primary" title="Expédition {{$expedi_clients->etat}}"><i class="fa fa-copy"></i> {{$expedi_clients->etat}}</span>
                                                                            @elseif($expedi_clients->etat == "")
                                                                                <span class="badge bg-light text-dark" title="Expédition non créée"><i class="fas fa-dolly"></i> Non créée</span>
                                                                            @endif   
                                                                        </td>                                                 
                                                                        <td class="text-start">
                                                                            @if($expedi_clients->etat_facture == "Brouillon")
                                                                                <span class="badge bg-secondary" title="Facture {{$expedi_clients->etat_facture}}"><i class="fa fa-box"></i> {{$expedi_clients->etat_facture}}</span>
                                                                            @elseif($expedi_clients->etat_facture == "Impayée")
                                                                                <span class="badge bg-danger" title="Facture {{$expedi_clients->etat_facture}}"><i class="fa fa-times-circle"></i> {{$expedi_clients->etat_facture}}</span>
                                                                            @elseif($expedi_clients->etat_facture == "Payée")
                                                                                <span class="badge bg-success" title="Facture {{$expedi_clients->etat_facture}}"><i class="fa fa-check-circle"></i> {{$expedi_clients->etat_facture}}</span>
                                                                            @elseif($expedi_clients->etat_facture == "Commencée")
                                                                                <span class="badge bg-info" title="Facture {{$expedi_clients->etat_facture}}"><i class="fa fa-recycle"></i> {{$expedi_clients->etat_facture}}</span>
                                                                            @elseif($expedi_clients->etat_facture == "Validée")
                                                                            <span class="badge bg-warning" title="Facture Non créer"><i class="fas fa-file-invoice-dollar"></i> Non créer</span>
                                                                            @endif                                                 
                                                                        </td> 
                                                                        <td class="pointer"><a href="detail_user?id={{$expedi_clients->user_id}}&active=12&champ=1-1" wire:navigate><i class="fas fa-user-circle text-bleu" aria-hidden="true"></i> {{$expedi_clients->nom_user}}</a></td>
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($expedi_clients->updated_at))}}</td>
                                                                        <td class="text-end"><a href="detail_expedition_clt?id={{$expedi_clients->id}}&ref={{$expedi_clients->code_expedition}}" wire:navigate class="btn-outline-muted"  title="Cliquez pour voir ou modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-eye"></i></a></td>
                                                                        {{-- <td class="taille_icon text-end">
                                                                            @if($confirmer === $expedi_clients->id)                                                               
                                                                                <a wire:click.prevent="supprimer({{$expedi_clients->id}},'{{$expedi_clients->code_facture}}')" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size:8px;"  title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer ?</i></a>
                                                                            @else
                                                                                <a wire:click.prevent="confirmerDelete({{$expedi_clients->id}})" class="btn btn-outline-muted btn-xs"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                            @endif
                                                                        </td>  --}}
                                                                    </tr>
                                                                @endforeach  
                                                        ...
                                                        <div class="bloc_pagination">{{$expedi_client->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
