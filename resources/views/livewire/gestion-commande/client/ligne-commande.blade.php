                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1 mb-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <a href="{{asset('exporter-cmd_client_ligne?date_debut='.$this->date_debut.'&date_fin='.$this->date_fin.'&produit='.$this->parNomProduit.'&client='.$this->query.'&ref='.$this->parCmd)}}" class="btn btn-sm btn-success mt-2 d-none d-md-block mb-1"><i class="fa-solid fa-file-export"></i> Exporter Excel</a>
                                                        ...
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="fond_entete_table pointer text-end" wire:click="setOrderField('created_at')">Commande <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                            ...
                                                            <tbody>
                                                                @foreach($cmd_client as $cmd_clients) 
                                                                    <tr>
                                                                        <td class="fw-semibold pointer"><a href="detail_product?id={{$cmd_clients->id_produit}}&active=4&champ=1-1&choix=2" wire:navigate class="text-bleu"><i class="fa fa-cube"></i> {{substr($cmd_clients->produit,0,52) > substr($cmd_clients->produit,0,51) ? substr($cmd_clients->produit,0,52).'...': $cmd_clients->produit}}</a></td>
                                                                        <td class="fw-semibold text-center">{{number_format($cmd_clients->quantite,0,',',' ')}}</td>
                                                                        <td class="fw-semibold pointer"><a href="nouveau_cmd_clt?id={{$cmd_clients->id_commande_client_entete}}&ref={{$cmd_clients->code_commande}}&active=6&champ=1-1&choix=2" wire:navigate><i class="fas fa-file-invoice-dollar text-danger" aria-hidden="true"></i> {{$cmd_clients->code_commande}}</a></td> 
                                                                        <td class="fw-semibold pointer"><a href="detail_tier?id={{$cmd_clients->id_client}}&ref={{$cmd_clients->code_commande}}&active=3&champ=3-2" wire:navigate class="text-bleu">@if($cmd_clients->nom_client) <i class="fa fa-user-circle"></i> @endif {{substr($cmd_clients->nom_client,0,52) > substr($cmd_clients->nom_client,0,51) ? substr($cmd_clients->nom_client,0,52).'...': $cmd_clients->nom_client}}</a></td>                                                                      
                                                                        <td class="fw-semibold text-end">{{date('d-m-Y', strtotime($cmd_clients->created_at))}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($cmd_clients->montant_ttc,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($cmd_clients->montant_remise,0,',',' ')}}</td>
                                                                        @if($autoriser == 1)
                                                                            <td class="fw-semibold text-end">{{number_format($cmd_clients->marge,0,',',' ')}}</td>
                                                                        @endif
                                                                        <td class="text-start">
                                                                            @if($cmd_clients->etat == "Brouillon")
                                                                                <span class="badge bg-secondary" title="Facture {{$cmd_clients->etat}}"><i class="fa fa-box"></i> {{$cmd_clients->etat}}</span>
                                                                            @elseif($cmd_clients->etat == "Validée")
                                                                                <span class="badge bg-success" title="Facture {{$cmd_clients->etat}}"><i class="fa fa-check-circle"></i> {{$cmd_clients->etat}}</span>
                                                                            @endif                                                 
                                                                        </td>     
                                                                        ...
                                                                        <td class="text-end"><a href="nouveau_cmd_clt?id={{$cmd_clients->id_commande_client_entete}}&ref={{$cmd_clients->code_commande}}&active=6&champ=1-1&choix=2" wire:navigate class=""  title="Cliquez pour modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-pencil"></i></a></td>
                                                                    </tr>
                                                                @endforeach 
                                                                <tr>
