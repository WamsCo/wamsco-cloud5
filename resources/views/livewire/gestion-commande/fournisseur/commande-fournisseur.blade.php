                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total commandes</span>
                                                    <span class="d-block fw-semibold text-dark" style="font-size: 13px;">{{$nbreTotalCmd}}</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home"><i class="fas fa-file-invoice text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$cmdFournisseurCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">  
                                            <button class="btn btn-sm btn-danger ms-auto" title="Cliquez pour enregistrer" data-toggle="tooltip" wire:click.prevent="store()"><i class="fa fa-plus-circle"></i> Nouvelle commande</button>                          
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <a href="{{asset('exporter-cmd_fourni?date_debut='.$this->date_debut.'&date_fin='.$this->date_fin.'&statut='.$this->parEtat.'&fournisseur='.$this->query.'&ref='.$this->parCmd)}}" class="btn btn-sm btn-success mt-2 d-none d-md-block mb-1"><i class="fa-solid fa-file-export"></i> Exporter Excel</a>
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="">
                                                                <select wire:model.live="parEtat" class="form-control form-select bordure w-auto"> 
                                                                    ...
                                                                    <option value="Validée">Validée</option>c
                                                                    <option value="Brouillon">Brouillon</option>
                                                                </select> 
                                                            </div>                            
                                                            <div>
                                                                <label for="query" class="sr-only">Recherche</label>
                                                                <input type="search" wire:model.live="query" id="query" class="form-control bordure" placeholder="Rech. fournisseur">
                                                            </div>
                                                            <div>
                                                                <label for="parCmd" class="sr-only">Recherche</label>
                                                                <input type="search" wire:model.live="parCmd" id="parCmd" class="form-control bordure" placeholder="Rech. référence">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    ...
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('code_facture')">Facture <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('etat_expedi')">Réception <i class="fa fa-arrow-down-short-wide"></i></th>        
                                                            ...
                                                            <tbody>
                                                                @foreach($cmd_fournisseur as $cmd_fournisseurs) 
                                                                    <tr>
                                                                        <td class="fw-semibold pointer"><a href="nouveau_cmd_fourni?id={{$cmd_fournisseurs->id}}&ref={{$cmd_fournisseurs->code_commande}}&active=6&champ=2-1&choix=1" wire:navigate><i class="fas fa-file-invoice text-danger" aria-hidden="true"></i> {{$cmd_fournisseurs->code_commande}}</a></td> 
                                                                        <td class="fw-semibold pointer"><a href="detail_tier?id={{$cmd_fournisseurs->id_fournisseur}}&ref={{$cmd_fournisseurs->code_commande}}&active=3&champ=3-2" wire:navigate class="text-bleu">@if($cmd_fournisseurs->nom_fournisseur) <i class="fa fa-user-circle"></i> @endif {{substr($cmd_fournisseurs->nom_fournisseur,0,52) > substr($cmd_fournisseurs->nom_fournisseur,0,51) ? substr($cmd_fournisseurs->nom_fournisseur,0,52).'...': $cmd_fournisseurs->nom_fournisseur}}</a></td>                                                                      
                                                                        <td class="">{{$cmd_fournisseurs->telephone}}</td>
                                                                        <td class="">{{date('d-m-Y', strtotime($cmd_fournisseurs->date_commande))}}</td>
                                                                        <td class="">{{date('d-m-Y', strtotime($cmd_fournisseurs->date_livraison))}}</td>
                                                                        <td class="">{{$cmd_fournisseurs->mode_reglement}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($cmd_fournisseurs->montant_ttc,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($cmd_fournisseurs->montant_recu,0,',',' ')}}</td>
                                                                        <td class="fw-semibold text-end">{{number_format($cmd_fournisseurs->reste_a_percevoir,0,',',' ')}}</td>
                                                                        @if($autoriser == 1)
                                                                            <td class="fw-semibold text-end">{{number_format($cmd_fournisseurs->marge,0,',',' ')}}</td>
                                                                        @endif
                                                                        <td class="text-start">
                                                                            @if($cmd_fournisseurs->etat == "Brouillon")
                                                                                <span class="badge bg-secondary" title="Commande {{$cmd_fournisseurs->etat}}"><i class="fa fa-box"></i> {{$cmd_fournisseurs->etat}}</span>
                                                                            @elseif($cmd_fournisseurs->etat == "Impayée")
                                                                                <span class="badge bg-danger" title="Commande {{$cmd_fournisseurs->etat}}"><i class="fa fa-times-circle"></i> {{$cmd_fournisseurs->etat}}</span>
                                                                            @elseif($cmd_fournisseurs->etat == "Payée")
                                                                                <span class="badge bg-success" title="Commande {{$cmd_fournisseurs->etat}}"><i class="fa fa-check-circle"></i> {{$cmd_fournisseurs->etat}}</span>
                                                                            @elseif($cmd_fournisseurs->etat == "Validée")
                                                                                <span class="badge bg-success" title="Commande {{$cmd_fournisseurs->etat}}"><i class="fa fa-check"></i> {{$cmd_fournisseurs->etat}}</span>
                                                                            @endif                                                 
                                                                        </td>  
                                                                        <td class="fw-semibold">
                                                                            @if(!empty($cmd_fournisseurs->code_facture))
                                                                                <a href="#" class="d-none d-md-block" wire:click.prevent="detailFact({{$cmd_fournisseurs->id_facture_fournisseur_entete}},'{{$cmd_fournisseurs->code_facture}}')"><span class="fw-semibold text-bleu" title="Cliquez pour voir les details facture fournisseur">@if($cmd_fournisseurs->code_facture) &nbsp;» @endif {{$cmd_fournisseurs->code_facture}}</span></a>
                                                                                {{-- <a href="nouveau_cmd_fourni?id={{$cmd_fournisseurs->id_facture_fournisseur_entete}}" class="text-bleu">@if($cmd_fournisseurs->code_facture)<i class="fa fa-university text-danger" aria-hidden="true"></i> @endif {{$cmd_fournisseurs->code_facture}}</a> --}}
                                                                            @else
                                                                                <span class="badge bg-warning" title="Facture non créée"><i class="fas fa-file-invoice-dollar"></i> Non créée</span>
                                                                            @endif
                                                                        </td> 
                                                                        <td class="text-start">
                                                                            @if($cmd_fournisseurs->etat_reception == "Brouillon")
                                                                                <span class="badge bg-secondary" title="Expédition {{$cmd_fournisseurs->etat_reception}}"><i class="fas fa-dolly"></i> {{$cmd_fournisseurs->etat_reception}}</span>
                                                                            @elseif($cmd_fournisseurs->etat_reception == "Clôturée")
                                                                                <span class="badge bg-danger" title="Expédition {{$cmd_fournisseurs->etat_reception}}"><i class="fas fa-dolly"></i> {{$cmd_fournisseurs->etat_reception}}</span>
                                                                            @elseif($cmd_fournisseurs->etat_reception == "Partiel")
                                                                                <span class="badge bg-primary" title="Expédition {{$cmd_fournisseurs->etat_reception}}"><i class="fas fa-dolly"></i> {{$cmd_fournisseurs->etat_reception}}</span>
                                                                            @elseif($cmd_fournisseurs->etat_reception == "")
                                                                                <span class="badge bg-light text-dark" title="Expédition non créée"><i class="fas fa-dolly"></i> Non créée</span>
                                                                            @endif   
                                                                        </td> 
                                                                        <td class="pointer"><a href="detail_user?id={{$cmd_fournisseurs->user_id}}&active=12&champ=1-1" wire:navigate><i class="fas fa-user-circle text-bleu" aria-hidden="true"></i> {{$cmd_fournisseurs->nom_user}}</a></td>                                           
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($cmd_fournisseurs->created_at))}}</td>
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($cmd_fournisseurs->updated_at))}}</td>
                                                                        <td class="text-end"><a href="nouveau_cmd_fourni?id={{$cmd_fournisseurs->id}}&ref={{$cmd_fournisseurs->code_commande}}" wire:navigate class=""  title="Cliquez pour modifier" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-pencil"></i></a></td>
                                                                        {{-- <td class="taille_icon text-end">
                                                                            @if($confirmer === $cmd_fournisseurs->id)                                                               
                                                                                <a wire:click.prevent="supprimer({{$cmd_fournisseurs->id}},'{{$cmd_fournisseurs->code_commande}}')" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size:8px;"  title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer ?</i></a>
                                                                            @else
                                                                                <a wire:click.prevent="confirmerDelete({{$cmd_fournisseurs->id}})" class="btn btn-outline-muted btn-xs"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                            @endif
                                                                        </td>  --}}
                                                                    </tr>
                                                                @endforeach 
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td colspan="5" class="text-end fw-semibold" style="color:#006666">{{number_format($montantTTC,0,',',' ')}}</td>
                                                        ...
                                                        <div class="bloc_pagination">{{$cmd_fournisseur->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
