<div x-data="{selection: @entangle('selection').defer}">
                                    ...
                                    <div class="card-body border border-light-subtle rounded-3 bg-white position-relative custom-scroll pe-2" style="height: calc(100vh - 131px); overflow-y: auto;">
                                        <div class="text-centerk">
                                            ...
                                            <div class="d-flex align-items-center justify-content-end gap-1">
                                                <div class="statusref">
                                                    @if($etat == "Brouillon")
                                                        <span class="badge bg-secondary py-1" title="Réception {{$etat}}"><i class="fa fa-box"></i> {{$etat}}</span>
                                                    @elseif($etat == "Clôturée")
                                                        <span class="badge bg-danger py-1" title="Réception {{$etat}}"><i class="fas fa-dolly"></i> {{$etat}}</span>
                                                    @elseif($etat == "Partiel")
                                                        <span class="badge bg-primary py-1" title="Réception {{$etat}}"><i class="fa fa-copy"></i> {{$etat}}</span>  
                                                    @elseif($etat == "Commencée")
                                                        <span class="badge bg-info py-1" title="Réception {{$etat}}"><i class="fa fa-recycle"></i> {{$etat}}</span>  
                                                    @endif
                                                </div>
                                                <div class="statusref">
                                        ...
                                        <div class="d-flex gap-1 mb-2">
                                            <a href="listing_reception_fourni?active=6&champ=2-1&choix=2" wire:navigate class="btn btn-sm btn-outline-danger flex-fill" title="Cliquez pour fermer" data-toggle="tooltip"><i class="fa fa-close"></i> Fermer</a>
                                            @if($confirmer === 2)                                                               
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1 border-bottom pb-2">
                                            @if($this->etat != "Clôturée")
                                                <a href="#" wire:click.prevent="clotureReception" target="_blank" class="btn btn-sm btn-outline-primary flex-fill fw-semibold ms-auto" title="Cliquez pour clôturer l'expédition"><i class="fa fa-dolly"></i> Clôturer la réception</a>
                                            @else
                                            ...
                                            <table class="table table-striped table-hover text-nowrap m-0">
                                                ...
                                                <tbody>
                                                    @foreach($cmdFourniEntete as $cmdFourniEntetes)
                                                        <tr>
                                                            <td class="fw-semibold"><a href="nouveau_cmd_fourni?id={{$cmdFourniEntetes->id}}&ref={{$cmdFourniEntetes->code_commande}}&active=6&champ=2-1&choix=1" wire:navigate>Cmd fourni.</a></td>                                                        
                                                    ...
                                                    @endforeach
                                                    @foreach($factFourniEntete as $factFourniEntetes)
                                                        <tr>
                                                            <td class="fw-semibold"><a href="nouveau_fact_fourni?id={{$factFourniEntetes->id}}&ref={{$factFourniEntetes->code_facture}}&active=7&champ=2-1&choix=1" wire:navigate>Facture client</a></td>
                                                            <td class="fw-semibold"><a href="nouveau_fact_fourni?id={{$factFourniEntetes->id}}&ref={{$factFourniEntetes->code_facture}}&active=7&champ=2-1&choix=1" wire:navigate class="text-bleu"><i class="fa fa-money-bill text-vert" aria-hidden="true"></i> {{$factFourniEntetes->code_facture}}</a></td>
                                                            <td class="fw-semibold text-end">{{number_format($factFourniEntetes->montant_ttc,0,',',' ')}} {{$this->devise}}</td>
                                                            <td class="fw-semibold"> 
                                                                @if($factFourniEntetes->etat == "Brouillon")
                                                                    <span class="badge bg-secondary" title="Facture {{$factFourniEntetes->etat}}"><i class="fa fa-box"></i> {{$factFourniEntetes->etat}}</span>
                                                                @elseif($factFourniEntetes->etat == "Impayée")
                                                                    <span class="badge bg-warning" title="Facture {{$factFourniEntetes->etat}}"><i class="fa fa-times-circle"></i> {{$factFourniEntetes->etat}}</span>
                                                                @elseif($factFourniEntetes->etat == "Payée")
                                                                    <span class="badge bg-success" title="Facture {{$factFourniEntetes->etat}}"><i class="fa fa-check-circle"></i> {{$factFourniEntetes->etat}}</span>
                                                                @elseif($factFourniEntetes->etat == "Commencée")
                                                                    <span class="badge bg-info" title="Facture {{$factFourniEntetes->etat}}"><i class="fa fa-recycle"></i> {{$factFourniEntetes->etat}}</span>
                                                                @endif                                                                                                                 
                                                            </td>
                                                            <td class="fw-semibold"><i class="fas fa-calendar-alt text-danger"></i> {{$factFourniEntetes->created_at ? date('d-m-Y H:i:s', strtotime($factFourniEntetes->created_at)): "" }}</td>                                                        
                                                        </tr>
                                                    @endforeach  
                                                </tbody>
                                            </table>  
                            ...
                            <div class="col-lg-9">
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-0">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                            <a href="listing_reception_fourni?active=6&champ=2-1&choix=2" wire:navigate class="btn btn-sm btn-bleu d-none d-md-block ms-auto"><i class="fa fa-chevron-left"></i> Retour liste</a>
                                                ...
                                                <div class="position-relative custom-scroll pe-3" style="height: calc(100vh - 357px); overflow-y: auto;">  
                                                    <div class="hauteur_ecran">
                                                        <div class="row pt-1 px-2">
                                                            ...
                                                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12"> 
                                                                <div class="row mb-1">
                                                                    ...
                                                                    <div class="col-md-9 col-sm-9">
                                                                        <span class="form-control sans_bordure fw-bold fs-6 text-vert">{{$this->codeReception}}</span>
                                                                    </div>
                                                                    <div class="d-flex justify-content-start">
                                                                        ...
                                                                        <div class="d-flex flex-column">
                                                                            ...
                                                                            <div  class="d-flex gap-1">
                                                                                @if($affiche === 1)
                                                                                    ...
                                                                                    <a href="#" wire:click.prevent="changeDateLivraison()" class="btn btn-sm btn-outline-muted border fw-semibold" data-bs-toggle="modal" data-bs-target="#saissieReglementModal" title="Cliquez pour modifier date initial" data-toggle="tooltip"><i class="fas fa-check"></i></a>
                                                                                @endif
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                            ...
                                                            <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">                                            
                                                                <div class="row mb-1">
                                                                    <label for="methode_reception" class="col-lg-6 col-md-6 col-sm-4 fw-bold col-form-label">Méthode réception <span wire:click.prevent="afficherMethodeReception(1)" title="Cliquez pour modifier la méthode de réception" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fas fa-pencil text-muted pointer"></i></span> </label>
                                                                    <div class="col-lg-6 col-md-6 col-sm-8">
                                                                        <div class="d-flex flex-column">
                                                                            <div>
                                                                                <span class="form-control sans_bordure fw-bold fs-6 text-bleu">{{$this->methode_reception}}</span>
                                                                            </div>
                                                                            <div class="d-flex gap-1">
                                                                                @if($ouvre === 1)
                                                                                    <select id="methode_reception" wire:model="methode_reception" class="form-control form-select bordure w-100 @error('methode_reception') is-invalid @enderror" id="methode_reception">
                                                                                        <option value=""></option>	
                                                                                    ...
                                                                                    <a href="#" wire:click.prevent="changeMethodeReception()" class="btn btn-sm btn-outline-muted border fw-semibold ms-auto" data-bs-toggle="modal" data-bs-target="#saissieReglementModal" title="Cliquez pour modifier la méthode de réception" data-toggle="tooltip"><i class="fas fa-check"></i></a>
                                                                                @endif
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <div class="d-flex justify-content-start">
                                                                        @error('methode_reception') <span class="text-danger">{{ $message }}</span> @enderror 
                                                                    </div>
                                                                </div>
                                                                <div class="row mb-1">
                                                                    ...
                                                                    <ul class="nav nav-tabs margin_ajuste border-0 pb-3" role="tablist">
                                                                        <li class="nav-item">
                                                                            @if(!empty($this->fact_entete_id))
                                                                                <a class="nav-link" data-bs-toggle="tab" href="#ligneRecept">Ligne de facture (<span class="text-vert">{{$recepClientLigneCount}}</span>)</a>
                                                                            @else
                                                                                <a class="nav-link" data-bs-toggle="tab" href="#ligneRecept">Ligne de commande (<span class="text-vert">{{$recepClientLigneCount}}</span>)</a>
                                                                            @endif
                                                                        </li>
                                                                        <li class="nav-item">
                                                                            <a class="nav-link active" data-bs-toggle="tab" href="#recept">Réception (<span class="text-vert">{{$recepClientLigneCount}}</span>)</a>
                                                                        </li>
                                                                    </ul>
                                                                    <div class="tab-content table-responsive border-top rounded-0">
                                                                        <div id="ligneRecept" class="tab-pane">                               
                                                                            <table class="table table-striped text-nowrap m-0"> 
                                                                                <thead>
                                                                                    <tr> 
                                                                                        <th class="fond_entete_table pointer" wire:click="setOrderField('produit')">Produit (<span class="text-vert">{{$recepClientLigneCount}}</span>) <i class="fa fa-arrow-down-short-wide"></i></th>
                                                                                ...
                                                                                <tbody>
                                                                                    @foreach($recepClient_ligne as $recepClient_lignes)
                                                                                        <tr> 
                                                                                            <td class="fw-bold"><a href="detail_product?id={{$recepClient_lignes->id_produit}}" wire:navigate class="text-bleu fw-bold">{{Str::limit($recepClient_lignes->produit, 22)}} - <span class="text-muted">{{Str::limit($recepClient_lignes->reference, 22)}}</span></a></td>                                                         
                                                                                            <td class="text-muted fw-semibold">{{$recepClient_lignes->type_produit}}</td>
                                                                                            <td class="fw-bold text-end">{{$recepClient_lignes->prix_achat}}</td>     
                                                                                            <td class="text-center fw-bold">{{$recepClient_lignes->quantite_recue}}</td>         
                                                                                            <td class="text-center fw-semibold @if($recepClient_lignes->remise > 0) text-vert @endif">{{$recepClient_lignes->remise}}%</td>         
                                                                                            <td class="text-end fw-semibold">{{$recepClient_lignes->tva}}%</td>         
                                                                                            <td class="text-center fw-semibold">{{$recepClient_lignes->precompte}}%</td> 
                                                                                            <td class="text-center fw-semibold @if($recepClient_lignes->offrir == "Oui") text-danger @endif">{{$recepClient_lignes->offrir}}</td> 
                                                                                            <td class="text-end fw-bold">{{number_format($recepClient_lignes->montant_ttc,0,',',' ')}}</td> 
                                                                                        </tr> 
                                                                                    @endforeach
                                                                                </tbody>
                                                                            </table>                                       
                                                                        </div>
                                                                        <div id="recept" class="tab-pane active">                               
                                                                            <table class="table table-striped table-hover text-nowrap m-0"> 
                                                                                <thead>
                                                                                    <tr>
                                                                                        ...
                                                                                        <th class="fond_entete_table text-center">Qté. reçue</th>
                                                                                ...
                                                                                <tbody> 
                                                                                    @foreach($recepClient_ligne as $recepClient_lignes)                                                    
                                                                                        <tr> 
                                                                                            <td class="fw-bold"><a href="detail_product?id={{$recepClient_lignes->id_produit}}&active=4&champ=1-1&choix=2" wire:navigate class="text-bleu fw-bold">{{Str::limit($recepClient_lignes->produit, 22)}} - <span class="text-muted">{{Str::limit($recepClient_lignes->reference, 22)}}</span></a></td>                                                         
                                                                                            <td class="text-muted fw-semibold">{{$recepClient_lignes->type_produit}}</td>
                                                                                            ...
                                                                                            <td class="text-center text-danger fw-semibold">{{$recepClient_lignes->reste_a_recevoir}}</td>  
                                                                                                @php
                                                                                            ...
                                                                                            <td class="text-start fw-semibold">
                                                                                                @if($recepClient_lignes->type_produit == "Produit")
                                                                                                    <a href="detail_entrepot?id={{$recepClient_lignes->id_entrepot}}&active=4&champ=3-1&choix=2" class="text-bleu" wire:navigate>{{$this->nom_entrepot}}
                                                ...
                                                <div class="alert mt-3 mb-0 d-flex justify-content-between align-items-center rounded-2 px-3 py-2" style="background-color: #f4f8ff; border: 1px solid #cce0ff;">
                                                    <div class="d-flex align-items-center gap-2">
                                                        ...
                                                        <span class="text-dark" style="font-size: 13px;">
                                                            <span class="fw-bold">Voir plus de réception,</span> 
                                                            <span class="text-muted ms-1">le statut d'un bon de commande.</span>
                                                        </span>
                                                    </div>
                                                    <a href="{{asset('listing_reception_fourni?active=6&champ=2-1&choix=2')}}" wire:navigate class="text-decoration-none fw-semibold text-primary" style="font-size: 13px;">Voir plus</a>
                                                </div>
                                            </div>                                                
                                        </div>                                            
                                    </div>                                     
    ...
    {{-- Modal --}}
    @include('livewire.gestion-commande.fournisseur.ajout_reception_partiel') 
</div>
