<div>
            ...
            <div class="col-sm-12">
                ...
                <div class="card mb-4 bg-2k">
                    <div class="card-header d-flex align-items-center justify-content-between">
                        <div>
                            <a href="#" class="btn btn-sm btn-secondary fw-bold text-white" title=" Cliquez pour ajouter" data-bs-toggle="modal" data-bs-target="#creerOppotuniteModal" data-toggle="tooltip" style="color: #161a1d;"><i class="fa fa-check-circle"></i> Nouvelle</a>
                        ...
                        <div class="d-flex align-items-center justify-content-between gap-1">
                            ...
                            <div class="d-flex align-items-center justify-content-between gap-1">                            
                                <a href="bienvenue?active=1" wire:navigate class="btn btn-sm btn-bleu ms-auto"><i class="fa fa-chevron-left"></i> Retour liste</a>
                    ...
                    <div class="card-body px-1 py-0 border-top" style="background-color: #f6f9fb;">
                                                ...
                                                <div class="w_kanban_header top-0 z-1 py-0 pt-print-0">
                                                    <div class="w_kanban_header_title position-relative d-flex lh-lg">
                                                        <div class="w_column_title flex-grow-1 min-w-0 mw-100 gap-1 d-flex fs-4 fw-bold align-top text-900">
                                                            <span class="text-truncate" title="{{$etapes->nom_etape}}">{{$etapes->nom_etape}}</span>
                                                        </div>
                                                        ...
                                                        <button class="w_kanban_quick_add d-print-none btn pe-2 me-n2" wire:click.prevent="tacher({{$etapes->id}})" data-bs-toggle="modal" data-bs-target="#creerOppotuniteModal">
                                                            <i class="fa fa-plus opacity-75 fw-bold" role="img" aria-label="Ajout rapide" title="Ajout rapide"></i>
                                                        </button>
                                                    </div>
                                                    @php                                                                       
                                                        $nombre = $this->getTotalParTache($etapes->id);                                                                 
                                                    @endphp
                                                    <div class="w_kanban_counter position-relative d-flex align-items-center justify-content-between mb-2">
                                                        ...
                                                        <div class="w_animated_number ms-2 text-900 text-nowrap cursor-default" data-tooltip="Revenu attendu"><b> {{$nombre}}</b></div>
                                                    </div>
                                                    <!-- ZONE DE DROP -->
                                                    <div class="dropzone min-vh-50 p-0 border rounded bg-white ecran_pipeline" ondrop="dropHandler(event, {{ $etapes->id }})" ondragover="dragOver(event)" ondragleave="dragLeave(event)">
                                                        @foreach ($tacher[$etapes->id] ?? [] as $task)                                                                
                                                            <div class="card rounded-0 p-0 shadow-sm slide-in w_kanban_record d-flex cursor-pointer @if($etapes->opacite == "Oui") opacity-50 @endif @if($dateJour > $task->date_cloture) bg-danger_opportunite @endif" wire:click.prevent="voirDetail({{$task->id}})" draggable="true" ondragstart="dragStart(event, {{ $task->id }})" ondragend="dragEnd(event)" wire:key="task-{{ $task->id }}" style="cursor: grab;">
                                                                <div class="card-body card-pipeline py-2">
                                                                    <span class="fw-bold fs-5">{{$task->nom_tache}}</span> <br/>
                                                                    @if($task->reference)
                                                                        <i class="fa fa-bullhorn opacity-75 fw-bold"></i> <span class="fw-semibold fs-6">{{$task->reference}}</span> <br/>
                                                                    @endif
                                                                    @php
                                                                        foreach($user as $users){
                                                                            if($task->id_utilisateur == $users->id){                                                                                    
                                                                                $profils = $users->profil;
                                                                                $nom_user = $users->name;
                                                                                $phone_user = $users->telephone;
                                                                                $type_user = $users->type_user;                                                                                
                                                                                $email_user = $users->email;                                                                                
                                                                            }
                                                                        } 
                                                                    @endphp  
                                                                    @if($task->temps_alloue)
                                                                        <div class="d-flex">
                                                                            <span class="text-truncate fw-semibold"><i class="fas fa-clock text-bleu"></i> {{$task->temps_alloue}}</span>
                                                                        </div>
                                                                    @endif
                                                                    <div class="d-flex">
                                                                        <span class="text-truncate fw-bold"><i class="fa fa-user-circle text-danger"></i> {{$nom_user}}</span>
                                                                    </div>
                                                                    @if($phone_user)
                                                                        <div class="d-flex">
                                                                            <span class="text-truncate"><i class="fa fa-phone text-danger"></i> {{$phone_user}}</span>
                                                                        </div>
                                                                    @endif 
                                                                    @if($email_user)
                                                                        <div class="d-flex">
                                                                            <span class="text-truncate"><i class="fa fa-envelope text-danger"></i> {{$email_user}}</span>
                                                                        </div>
                                                                    @endif  
                                                                    ...
                                                                    <footer class="pt-1">                                          
                                                                        <div class="ms-auto d-flex justify-content-between align-items-center">
                                                                            <div class="fw-semibold" title="Date clôture tâche" ><i class="fa fa-calendar-alt text-bleu"></i> {{date('d-m-Y', strtotime($task->date_cloture))}} <i class="fas fa-clock text-danger"></i></div>
                                                                            <div class="d-flex">
                                                                            ...
                                                                            <div class="image_user">                                                                           
                                                                                @if($profils != null)
                                                                                    <img class="rounded-circle" src="storage/{{$profils}}" title="{{$type_user}} » {{$nom_user}} @if($phone_user)» {{$phone_user}} @endif" alt="Profil"/>
                                                                                @else
                                                                                    <img class="rounded-circle" src="storage/default/user_man.png" title="{{$type_user}} » {{$nom_user}} @if($phone_user)» {{$phone_user}} @endif" alt="Profil"/>
                                                                                @endif
                                                                            </div>
                                                                        </div>
                                                                    </footer>
    ...
    {{-- Modal --}}
    @include('livewire.gestion-tache.creation_tache') 
    <script>
