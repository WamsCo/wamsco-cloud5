<div x-data="{selection: @entangle('selection').defer}">
                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">                                                
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total Inventaire</span>
                                                    <span class="d-block fw-semibold text-dark" style="font-size: 13px;">{{number_format($nbreTotalInventaire,0,',',' ')}}</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home" title="Liste des tieres : Prospects, Clients, Fournisseurs"><i class="fas fa-users text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$inventaireCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">                            
                                            <button href="#" data-bs-toggle="modal" data-bs-target="#createInventaireModal" title="Cliquez pour ajouter un nouveau tier" data-toggle="tooltip" class="btn btn-sm btn-danger ms-auto"><i class="fa fa-plus-circle"></i> Nouveau invetaire</button>
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="">
                                                                <select wire:model.live="parEntrepot" class="form-control form-select bordure w-100"> 
                                                    ...
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    <th class="fond_entete_table text-start pointer" wire:click="setOrderField('reference')">Réf. <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table text-start pointer" wire:click="setOrderField('date_inventaire')">Date Inventaire <i class="fa fa-arrow-down-short-wide"></i></th>                 
                                                                    ...
                                                                    <th class="fond_entete_table text-start pointer" wire:click="setOrderField('entrepot')">Entrepôt <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                            ...
                                                            <tbody>
                                                                @foreach($inventaire as $inventaires) 
                                                                <tr>  
                                                                    <td class="text-start"><a href="detail_inventaire?id={{$inventaires->id}}&active=4&champ=3-1&choix=7" class="text-bleu fw-semibold" wire:navigate><i class="fa fa-dolly"></i> {{$inventaires->reference}}</a></td>                                             
                                                                    <td class="text-start fw-semibold">{{date('d-m-Y', strtotime($inventaires->date_inventaire))}}</td>
                                                                    <td class="text-start fw-semibold">{{$inventaires->libelle}}</td> 
                                                                    <td class="text-start"><a href="detail_entrepot?id={{$inventaires->id_entrepot}}" class="text-bleu fw-semibold" wire:navigate><i class="fa fa-box-open text-danger"></i> {{$inventaires->entrepot}}</a></td>  
                                                                    @if($inventaires->etat == "Brouillon")
                                                                        <td class="text-start text-black"><span class="badge bg-secondary"><i class="fa fa-box"></i> {{$inventaires->etat}}</span></td>                                          
                                                                    @elseif($inventaires->etat == "Clôturé")
                                                                        <td class=""><span class="badge bg-success"><i class="fa fa-check-circle"></i> {{$inventaires->etat}}</span></td>
                                                                    @endif 
                                                                    <td class="text-start">{{$inventaires->nom_user}}</td> 
                                                                    <td class="text-start">{{date('d-m-Y H:i:s', strtotime($inventaires->created_at))}}</td>
                                                                    <td class="taille_icon text-end">
                                                                        @if($confirmer === $inventaires->id)                                                               
                                                                            <a wire:click.prevent="supprimer({{$inventaires->id}})" class="btn btn-outline-danger btn-xs bg-danger text-white blink" style="font-size: 8px;" title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer?</i></a>
                                                                        @else
                                                                            <a wire:click.prevent="confirmerDelete({{$inventaires->id}})" class="btn-outline-muted btn-xs"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                        @endif
                                                                    </td> 
                                                                </tr>
                                                                @endforeach 
                                                        ...
                                                        <div class="bloc_pagination">{{$inventaire->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    {{-- Modal --}}
    @include('livewire.gestion-stock.inventaires.nouv-inventaire')  
</div>
