<div>
	<div wire:ignore.self class="modal fade" id="imageProduitModal" tabindex="-1" role="dialog" aria-labelledby="slidemodalLabel" aria-hidden="true">
                        ...
                        <div class="card-body">
                            <div class="m-0">
                              ...
                              <div class="text-center p-3">
                                    @if($image)                                 
                                        <img class="w-full h-32 rounded-lg" style="width: 72px;border-radius: 50%;" src="{{ $image->temporaryUrl() }}">
                                    @elseif($old_image)
                                        <img class="w-full h-32 rounded-lg" style="width: 72px;border-radius: 50%;" src="storage/{{$old_image}}">
                                    @else       
                                        <img class="w-full h-32 rounded-lg" style="width: 72px;border-radius: 50%;" src="storage/default/image.png">                     
                                    @endif 
                              </div>
                              <input type="file" wire:model="image" class="form-control @error('image') is-invalid @enderror"  id="image">
                              @error('image')<div class="text-danger invalid-feedback">{{ $message }}</div>@enderror
                            </div>
                        </div>
					</div>
