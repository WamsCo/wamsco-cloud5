<div x-data="{selection: @entangle('selection').defer}">
                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">                                                
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total Nomenclature</span>
                                                    <span class="d-block fw-semibold text-dark" style="font-size: 13px;">Actif : <span class="text-danger">{{number_format($nbreTotalNomenclature,0,',',' ')}}</span> / {{number_format($nbreTotalNomenclatureTotal,0,',',' ')}}</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home" title="Listing {{$title_fils}}"><i class="fas fa-university text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$listeNomenclatureCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1"> 
                                            <button data-bs-toggle="modal" data-bs-target="#createNomenclatureModal" title="Cliquez pour voir les détails" data-toggle="tooltip" class="btn btn-sm btn-danger ms-auto"><i class="fa fa-plus-circle"></i> Nouvelle nomenclature</button>
                                        </div>
                                    </div>
                                </div>
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    ...
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('code')">Référence <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('libelle')">Nomenclature <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    ...
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('type_nomencla')">Type nomenclature <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                            ...
                                                            <tbody>
                                                                @foreach($listeNomenclatur as $listeNomenclaturs) 
                                                                    <tr>                                            
                                                                        {{-- <td class="">#{{$listeNomenclaturs->id}}</td> --}}
                                                                        <td class=""><a href="detail_nomencla?id={{$listeNomenclaturs->id}}&active=9&champ=1-1&choix=2" wire:navigate><span style="color:#1c4914; font-weight: 700;"><i class="fa fa-shapes"></i> {{Str::limit($listeNomenclaturs->code, 32)}}</span></a></td>    
                                                                        <td class=""><a href="detail_nomencla?id={{$listeNomenclaturs->id}}&active=9&champ=1-1&choix=2" wire:navigate class="text-muted fw-semibold">{{Str::limit($listeNomenclaturs->libelle, 32)}}</a></td>    
                                                                        <td valign="middle" class="fw-bold">
                                                                            <a href="detail_product?id={{$listeNomenclaturs->produit_id}}&active=4&champ=1-1&choix=2" wire:navigate class="text-bleu"><i class="fa fa-cube"></i> {{Str::limit($listeNomenclaturs->produit_a_fabrique, 32)}}</a>
                                                                        </td>                                                                      
                                                                        <td valign="middle" class="fw-semibold">{{$listeNomenclaturs->type_nomencla}}</td>                              
                                                                        <td valign="middle" class="text-center" style="font-weight: 700;"> {{$listeNomenclaturs->quantite}} <span style="font-weight: 500;">{{$listeNomenclaturs->unite_mesure}}</span></td> 
                                                                        <td valign="middle" class="text-center"><span style="color:#f66161; font-weight:600;"><i class="fa fa-calendar" data-toggle="tooltip"></i> {{$listeNomenclaturs->duree}}</td> 
                                                                        <td valign="middle" class="fw-semibold">{{$listeNomenclaturs->entrepot_fabrication}}</td> 
                                                                        <td valign="middle" class="pointer text-center" wire:click.prevent="changeEtat({{$listeNomenclaturs->id}}, '{{$listeNomenclaturs->etat}}')" title="Cliquez pour changer l'état">
                                                                            @if($listeNomenclaturs->etat == 1)
                                                                        ...
                                                                        <td valign="middle" class="">{{Str::limit($listeNomenclaturs->description, 32)}}</td> 
                                                                        <td class="">
                                                                            <a href="detail_user?id={{$listeNomenclaturs->user_id}}&active=12&champ=1-1" wire:navigate class="text-muted fw-semibold"><i class="fa fa-user-circle"></i> {{Str::limit($listeNomenclaturs->nom_user, 22)}}</a>
                                                                        </td>
                                                                        <td class="">{{date('d-m-Y H:i:s', strtotime($listeNomenclaturs->created_at))}}</td>
                                                                        <td valign="middle" class="">
                                                                            @if($listeNomenclaturs->code)
                                                                                <a href="detail_nomencla?id={{$listeNomenclaturs->id}}&active=9&champ=1-1&choix=2" wire:navigate class="btn-outline-muted"><i class="fa fa-eye" title="Cliquez pour voir ou modifier" data-toggle="tooltip"></i></a>
                                                                            @endif
                                                                        </td>
                                                                        <td valign="middle" class="pointer">
                                                                            @if($confirmer === $listeNomenclaturs->id)
                                                                                <a wire:click.prevent="supprimerAll({{$listeNomenclaturs->id}},'{{$listeNomenclaturs->type_listeNomenclaturs}}')" style="font-size: 8px;"><i class="fa fa-trash bg-red text-white w-32 px-1 py-1 blink rounded border" title="Cliquez pour confirmer la suppression" data-toggle="tooltip"> Confirmer?</i></a>
                                                                            @else
                                                                                <a wire:click.prevent="confirmerDeleted({{$listeNomenclaturs->id}})"><i class="fa fa-trash btn-outline-muted" title="Cliquez pour supprimer" data-toggle="tooltip"></i></a>
                                                                            @endif
                                                                        </td> 
                                                                    </tr>
                                                                @endforeach 
                                                        ...
                                                        <div class="bloc_pagination">{{$listeNomenclatur->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    {{-- Modal --}}
    @include('livewire.fabrication.create_nomenclature')    
</div>
