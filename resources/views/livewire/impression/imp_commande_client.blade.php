        ...
        <div class="container largeur">
             ...
             <div style="display:flex; justify-content:space-between; margin-bottom:10px; margin-top:4px;">
                ...
                <div style="width:80%; display:flex; justify-content:space-between; gap:10px;padding: 0 4px;">
                    <div style="display:flex; flex-direction:column;">                       
                        <span style="font-size: 23px; font-weight: 700;">@if($code_proforma == "vide") Commande @else Proforma @endif</span>                        
                    ...
                    <div style="display:flex; flex-direction:column;">                        
                        <span class="fact_num">@if($code_proforma == "vide") Commande N°: {{$code_commande}} @else Proforma N°: {{$code_proforma}} @endif</span>                        
                        <span class="fact" style="font-size: 13px;padding-top: 6px;">Date @if($code_proforma == "vide") Commande @else Proforma @endif: <span class="donnees">{{date('d/m/Y, H:i:s', strtotime($date_vente))}}</span></span>
                        <span class="fact" style="font-size: 13px;">Caissier-e: <span class="donnees">{{substr($nom_user,0,15)}}</span></span> 
                        @if($code_proforma == "vide")                        
                            @if($code_proformaOk)                        
                                <span class="fact_numk" style="font-weight: bold;font-size: 12px;color: #27852f;">Réf. Proforma: {{$code_proformaOk}}</span>
                            @endif                      
                        @endif                      
                    </div>
                </div>
            ...
            <div style="display: flex; justify-content: space-between;margin-top:10px; margin-bottom: 14px; gap:5px;">
                <div style="width:50%; display:flex; flex-direction:column; padding:0 4px;">
                    ...
                    <span style="">Statut de la @if($code_proforma == "vide") Commande @else Proforma @endif: 
                        @if($statut_cmd == "Validée")
                            <span class="donnees" style="color:#274085">{{$statut_cmd}}</span>                       
                        @else
                            <span class="donnees">{{$statut_cmd}}</span>
                        @endif
                    </span>
                    @if($condition_reglement)
                        <span style="">Conditions de règlement:                        
                            <span class="donnees text-success" style="font-size: 14px;">{{$condition_reglement}}</span>                        
                        </span>
                    @endif
                    <span style="">Date prévue de livraison:                        
                        <span class="donnees" style="font-size: 14px; color:#274085">{{date('d/m/Y', strtotime($date_livraison))}}</span>                        
                    </span>
                    <span style="">Mode de règlement:                        
                        <span class="donnees text-success" style="font-size: 14px;">{{$mode_reglement}}</span>                        
                    </span>
                </div>
                <div style="width:50%; display:flex; flex-direction:column; padding: 4px 40px; background:#e3e3e3">
                    ...
                    @endif
                    <span style="display: flex;flex-direction: row; justify-content: space-between; align-items: center;font-weight: 500;margin-top: 10px;">Total TTC: <span class="donnees" style="font-weight: 700;color: #2c299a;border-top: 1px solid;">{{number_format($montant_ttc,0,' ',' ')}} {{$devise}}</span></span>
                </div>
            </div>            
