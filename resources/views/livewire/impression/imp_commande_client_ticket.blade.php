        ...
        <div class="bloc_facture_mini">
            <div style="display: flex;flex-direction: column;">                    
                @foreach($entite as $entites) 
                    ...
                    @endif  
                @endforeach                          
                <div class="entete_fact_mini" style="border-top: 1px dotted;margin-top: 3px;padding-top: 2px;">@if($code_proforma == "vide") Commande N°: <span style="font-weight: 600;" >#{{$code_commande}}</span> @else Proforma N°: <span style="font-weight: 600;" >#{{$code_proforma}}</span> @endif</div> 
                <div class="entete_fact_mini">Date @if($code_proforma == "vide") Commande @else Proforma @endif: <span class="data_fact_mini">{{date('d/m/Y, H:i:s', strtotime($date_vente))}}</span></div>  
                <div class="entete_fact_mini">Client: 
                    @if($sexe == "Masculin")
                    ...
                    <tfoot>                  
                        ...
                        <tr>
                            <td colspan="4">
                                ...
                                <div class="titre_fact_mini py-1" style="font-weight: 600;">Statut de la @if($code_proforma == "vide") Commande @else Proforma @endif: 
                                ...
                                @if($condition_reglement)
                                    <div class="titre_fact_mini py-1" style="font-weight: 600;">Conditions de règlement:                                    
                                        <span class="donnees text-success">{{$condition_reglement}}</span> 
                                    </div>
                                @endif
                                <div class="titre_fact_mini py-1" style="font-weight: 600;">Date prévue de livraison:                                    
                                    <span class="donnees" style="color:#274085">{{date('d/m/Y', strtotime($date_livraison))}}</span> 
                                </div>
                                <div class="titre_fact_mini py-1" style="font-weight: 600;">Mode de règlement:                                     
                                    <span class="donnees text-success">{{$mode_reglement}}</span> 
                                </div>                                                                    
                            </td>
                        </tr>
                    </tfoot>
