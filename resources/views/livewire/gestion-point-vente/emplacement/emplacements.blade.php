<div x-data="{selection: @entangle('selection').defer}" wire:poll.visible.30s>
                            ...
                            <div class="col-lg-12">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between flex-nowrap overflow-auto py-2">
                                            ...
                                            <div class="d-flex align-items-center px-3 border-end flex-fill">                                                
                                                ...
                                                <div class="text-nowrap">
                                                    <span class="d-block fw-bold text-secondary" style="font-size: 11px;">Total emplacement</span>
                                                    <span class="d-block fw-semibold text-dark" style="font-size: 13px;">{{number_format($nbreTotalEmplacement,0,',',' ')}}</span>
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-1">
                                    ...
                                    <ul class="nav nav-tabs margin_ajuste border-0" role="tablist">
                                        <li class="nav-item">                                                
                                            <a class="nav-link active" data-bs-toggle="tab" href="#home" title="Liste des tieres : Prospects, Clients, Fournisseurs"><i class="fas fa-users text-bleu"></i> {{$title_fils}} (<span class="text-vert">{{$emplacementCount}}</span>)</a>                                                
                                        </li>  
                                        <li class="nav-item">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">                            
                                            <button class="btn btn-sm btn-danger ms-auto" title="Cliquez pour ajouter" data-toggle="tooltip" data-bs-toggle="modal" data-bs-target="#createEmplacementModal"><i class="fa fa-plus-circle"></i> Nouveau emplacement</button>
                                                ...
                                                <div class="card shadow-sm border border-light-subtle rounded-3 bg-white px-0">
                                                    <div class="d-flex align-items-center justify-content-between gap-3 pt-2 px-2">
                                                        <div class="d-flex align-items-center justify-content-between gap-1">
                                                            ...
                                                            <div class="d-flex align-items-center justify-content-between gap-1">
                                                                <h6><a href="{{asset('exporter-emplacement')}}" class="btn btn-sm btn-success mt-2 d-none d-md-block"><i class="fa-solid fa-file-export"></i> Exporter Excel</a></h6>
                                                        ...
                                                        <div class="d-flex align-items-center justify-content-between gap-1">    
                                                            <div>
                                                                ...
                                                                <input type="search" wire:model.live="query" id="query" class="form-control bordure" placeholder="Rech. emplacement">
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="table-responsive border-top">
                                                        <table class="table table-striped table-hover text-nowrap m-0"> 
                                                            <thead>
                                                                <tr>
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('nom_emplacement')">Emplacement <i class="fa fa-arrow-down-short-wide"></i></th>   
                                                                    ...
                                                                    <th class="fond_entete_table pointer" wire:click="setOrderField('non_caissiere')">Caissier-ère <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                                    <th class="fond_entete_table pointer text-start" wire:click="setOrderField('utiliser')">Utiliser <i class="fa fa-arrow-down-short-wide"></i></th> 
                                                            ...
                                                            <tbody>
                                                                @foreach($emplacement as $emplacements) 
                                                                    <tr>
                                                                        <td class="fw-bold pointer text-bleu" wire:click.prevent="edit('{{$emplacements->id}}')" data-bs-toggle="modal" data-bs-target="#updateEmplacementModal" title="Cliquez pour voir les détails" data-toggle="tooltip"><i class="fa fa-table text-danger" aria-hidden="true"></i> {{$emplacements->nom_emplacement}}</td> 
                                                                        <td class="">{{$emplacements->description}}</td>
                                                                        <td style="font-weight: 700; color:#707070;">{{Str::limit($emplacements->non_caissiere, 52)}}</td>
                                                                        <td style="font-weight: 600; color:#707070;" class="fw-bold text-start">
                                                                            @if($emplacements->utiliser == "Oui")                                             
                                                                                <span class="text-danger"><i class="fa fa-refresh"></i> Encours</span>
                                                                            @else
                                                                                <span class="text-green"><i class="fa fa-check-circle"></i> Disponible</span>                                        
                                                                            @endif   
                                                                        </td>                                                            
                                                                        <td class="fw-semibold text-muted pointer">@if($emplacements->nom_user)<i class="fa fa-user-circle" aria-hidden="true"></i>@endif {{Str::limit($emplacements->nom_user, 52)}}</td>                                                                      
                                                                        <td class="fw-semibold text-muted">{{date('d-m-Y H:i:s', strtotime($emplacements->created_at))}}</td>
                                                                        <td class="text-end pointer" wire:click.prevent="edit('{{$emplacements->id}}')" data-bs-toggle="modal" data-bs-target="#updateEmplacementModal" title="Cliquez pour voir les détails" data-toggle="tooltip"><i class="fa fa-pencil btn-outline-muted"></i></td>
                                                                        <td class="taille_icon text-end">
                                                                            @if($confirmer === $emplacements->id)                                                               
                                                                                <a wire:click.prevent="supprimer({{$emplacements->id}},'{{$emplacements->code_facture}}')" class="btn btn-outline-danger btn-xs bg-danger text-white pointer" style="font-size:8px;"  title="Cliquez pour confirmer la suppression" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"> Confirmer ?</i></a>
                                                                            @else
                                                                                <a wire:click.prevent="confirmerDelete({{$emplacements->id}})" class="btn-outline-muted pointer"  title="Cliquez pour supprimer" data-bs-toggle="tooltip" data-bs-placement="top"><i class="fa fa-trash"></i></a>
                                                                            @endif
                                                                        </td> 
                                                                    </tr>
                                                                @endforeach
                                                        ...
                                                        <div class="bloc_pagination">{{$emplacement->links()}}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    {{-- Modal --}}
    @include('livewire.gestion-point-vente.emplacement.create')
    @include('livewire.gestion-point-vente.emplacement.update')    
</div>
