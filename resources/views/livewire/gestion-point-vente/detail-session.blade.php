                                    ...
                                    <div class="card-body border border-light-subtle rounded-3 bg-white position-relative custom-scroll pe-2" style="height: calc(100vh - 131px); overflow-y: auto;">
                                        ...
                                        <div class="d-flex gap-2">
                                            @if($etat != "Contrôle à l'ouverture")
                                                <a href="{{asset('pos_sessions?active=5&champ=1-1')}}" title="Cliquez pour fermer" data-toggle="tooltip" class="btn btn-sm btn-outline-danger flex-fill fw-semibold mb-2"><i class="fa fa-close"></i> Fermer</a>
                                            @endif 
                                        </div>
                                        <div class="d-flex gap-2 mb-3 border-bottom pt-0 pb-2">
                                            @if($etat == "Contrôle à l'ouverture")
                                                <button href="{{asset('pos_sessions?active=5&champ=1-1')}}" wire:navigate class="btn btn-sm btn-outline-danger flex-fill" title="Cliquez pour fermer"><i class="fa fa-close"></i> Fermer</button>
                                            @endif 
                                            @if($this->verifie_id == auth()->user()->id)                                                  
                                                @if(!empty($date_ouverture))
                                                    @if($etat != "Clôturée")
                                                        <a href="pos?id={{$ids}}&ref={{$session_id}}" wire:navigate class="btn btn-sm btn-outline-green fw-semibold flex-fill"><i class="fa fa-refresh fa-spin"></i> Continuer la vente</a>
                                                    @endif
                                                @else 
                                                    @if($etat != "Clôturée")
                            ...
                            <div class="col-lg-9">
                                ...
                                <div class="card-header d-flex align-items-center justify-content-between border-0 px-0">
                                    ...
                                    <div class="d-flex align-items-center justify-content-between gap-1">
                                        ...
                                        <div class="d-flex align-items-center justify-content-between gap-1">                            
                                            <a href="{{asset('pos_sessions?active=5&champ=1-1')}}" wire:navigate class="btn btn-sm btn-bleu d-none d-md-block ms-auto"><i class="fa fa-chevron-left"></i> Retour liste</a>
                                                ...
                                                <div class="alert mt-3 mb-0 d-flex justify-content-between align-items-center rounded-2 px-3 py-2" style="background-color: #f4f8ff; border: 1px solid #cce0ff;">
                                                    <div class="d-flex align-items-center gap-2">
                                                        ...
                                                        <span class="text-dark" style="font-size: 13px;">
                                                            <span class="fw-bold">Voir plus de session ,</span> 
                                                            <span class="text-muted ms-1">un aperçu de vos sessions y compris les informations sur les transactions.</span>
                                                        </span>
                                                    </div>
                                                    <a href="{{asset('pos_sessions?active=5&champ=1-1')}}" wire:navigate class="text-decoration-none fw-semibold text-primary" style="font-size: 13px;">Voir plus</a>
                                                </div>
                                            </div>                                                
                                        </div>                                            
                                    </div> 
