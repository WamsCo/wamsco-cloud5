    ...
    <div class="pos dvh-100 d-flex flex-column position-fixed w-100" wire:poll.visible.20s> 
        ...
        <div class="pos-topheader position-relative navbar-height d-flex align-items-center justify-content-between p-2 m-0 bg-view border-bottom">
            <div class="pos-leftheader d-flex align-items-center gap-2">
                ...
                @endif
                <button class="btn btn-sm btn-white" wire:click.prevent="afficheTable()" title="Cliquez pour voir les tables" data-toggle="tooltip"><i class="fa fa-table"></i> Tables</button>
                @if($activer_ecran_cuisine == 1)
                    <button class="btn btn-sm btn-white fw-bold rounded pointer" wire:click.prevent="CmdAttente()" title="Voir la liste des commandes"><i class="fa fa-list-ul"></i> Commandes</button>
                @endif
                <button class="btn btn-sm btn-white text-bleu fw-bold rounded-pill cursor-default no-hover" title="Votre table actuelle"><i class="fa fa-cutlery"></i> {{$this->nom_table}}</button>
            ...
            <div class="block_pos h-100">
                <div class="leftpane d-flex flex-column pb-2 border-endk">                
                            ...
                            {{-- <table class="col table table-hover text-nowrap"> --}}
                                <thead style="background: #ffffff !important; color: rgb(0, 0, 0) !important; font-size: 12px;">  
                                    <tr style="position: sticky;top: 0;background: #ffffff;">
                                        <th>Produit</th>                                                        
                                        <th colspan="5">
                                            @if($statut_cmd == "Terminer")
                                                <span class="btn btn-xs btn-white text-vert fw-bold rounded-pill cursor-default no-hover"><i class="fa fa-thumbs-up"></i> {{$statut_cmd}}</span>
                                            @elseif($statut_cmd == "A préparer")
                                                <span class="btn btn-xs btn-white fw-bold rounded-pill cursor-default no-hover" style="color: #ff5a00;"><i class="fa fa-spinner fa-spin"></i> {{$statut_cmd}}</span>
                                            @elseif($statut_cmd == "En cours")
                                                <span class="btn btn-xs btn-white text-primary fw-bold rounded-pill cursor-default no-hover" style="color: #456bf2;"><i class="fa fa-hand"></i> {{$statut_cmd}}</span>
                                            @else
                                                {{-- <span class="btn btn-xs btn-white text-bleu fw-bold rounded-pill cursor-default no-hover"></span> --}}
                                            @endif 
                                        </th>                                                        
                    ...
                    <div class="pads px-0 bloc_paie">                    
                        ...
                        <div class="d-flex gap-2 w-100 py-2 px-1 flex-wrap">
                            ...
                            @if($test_vide > 0)
                                <a href="facturationclt-pdf?id={{$idFclt}}&code={{$code_fact}}&format=A4&pos=pv_restau" class="btn btn-light btn-lg lh-lg text-black flex-shrink-0 rounded-1 px-2" title="Cliquez pour imprimer A4" data-toggle="tooltip" target="_blank"><i class="fa fa-print"></i>A4</a>
                                <a href="facturationclt-pdf?id={{$idFclt}}&code={{$code_fact}}&format=ticket&pos=pv_restau" class="btn btn-light btn-lg lh-lg text-black flex-shrink-0 rounded-1 px-2" title="Cliquez pour imprimer ticket" data-toggle="tooltip" target="_blank" style="color: #161a1d;"><i class="fa fa-print"></i>Ticket</a>
                            ...
                            <div class="d-flex flex-column gap-2">
                                <div class="validation d-flex gap-2 px-1">
                                    @if($test_vide > 0)
                                        ...
                                        @else
                                        @if($activer_ecran_cuisine == 1)
                                            <button class="button btn btn-bleu btn-lg py-3 d-flex align-items-center justify-content-center flex-fill rounded-1" wire:click="EnvoyerCuisine()" title="Cliquez pour envoyer en cuisine" data-toggle="tooltip">Envoyer en cuisine</button>
                                        @endif
                                        <button class="button btn btn-light text-bleu btn-lg py-3 d-flex align-items-center justify-content-center flex-fill rounded-1" wire:click="AffichePaie()" data-bs-toggle="modal" data-bs-target="#saissieReglementPosModal" title="Cliquez pour effectuer le paiement" data-toggle="tooltip">Paiement</button>
                                        @endif
                                    @endif
                                </div>
                            </div>
                ...
                <div class="rightpane d-flex flex-column flex-grow-1 bg-100">
                    <div class="px-1 py-1 gap-2 bg-view choix_catg">
                        <a href="#" class="btn btn-sm btn-black my-1" wire:click.prevent="filtreAll()" title="Cliquez pour afficher toutes les produits" data-toggle="tooltip"><i class="fa fa-refresh"></i></a>
                        @foreach($categorieProd as $categorieProds)    
                            <a href="#" class="btn btn-sm btn-black my-1" wire:click.prevent="filtres('{{$categorieProds->nom_categorie}}')" title="Cliquez pour filtrer les catégories" data-toggle="tooltip"><i class="fa fa-clone"></i> {{Str::limit($categorieProds->nom_categorie, 22)}}</a>
                        @endforeach
                    </div>
                    <div class="px-1 py-1 gap-2">
                        <div class="bloc_select_prod">  
                            @foreach($categorieProd as $categorieProds)                  
                                        ...
                                        <article class="product" wire:click.prevent="choisir({{$stockProds->id}})">                                   
                                            <div class="product-img">
                                                @if($stockProds->image != null)
                                                    <img class="" src="storage/{{$stockProds->image}}">
                                                @else
                                                    <img class="" src="storage/default/image.png">
                                                @endif
                                                <span class="price-tag">{{$stockProds->prix_vente_unitaire}} {{$devise}}</span>
