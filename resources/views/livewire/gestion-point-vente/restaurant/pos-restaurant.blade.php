                ...
                <div class="pos dvh-100 d-flex flex-column position-fixed w-100" wire:poll.visible.30s> 
                    ...
                    <div class="pos-topheader bg-view border-bottom">                        
                        <div class="pos-leftheader">
                            ...
                            @endif
                            {{-- Backend --}}
                            <a href="detail_restau_session?id={{ $this->id_session_restau }}&ref={{ $this->ref_session_restau }}&active=5&champ=2-1&choix=1" class="btn btn-sm btn-white pos-action-btn" wire:navigate title="Retourner à la session {{ $this->ref_session_restau }}"><i class="fa fa-refresh"></i> <span class="d-none d-lg-inline">Backend</span></a>
                            <button class="btn btn-sm btn-white text-danger fw-bold rounded-pill cursor-default no-hover" title="Votre table actuelle"><i class="fa fa-cutlery"></i> {{$this->nom_table}}</button>
                        </div>                       
                            ...
                            <div class="pos-cart-layout">                                
                                ...
                                <section class="pos-bottom-section">   
                                    ...
                                    <div class="pos-tools">
                                        ...
                                        @if($test_vide > 0)
                                            {{-- A4 --}}
                                            <a href="facturationclt-pdf?id={{$idFclt}}&code={{$code_fact}}&format=A4&pos=pv_restau" target="_blank" class="pos-toolk btn btn-sm btn-outline-secondary flex-fill fw-semibold ms-auto"><i class="fa fa-print"></i> A4</a>
                                            {{-- TICKET --}}
                                            <a href="facturationclt-pdf?id={{$idFclt}}&code={{$code_fact}}&format=ticket&pos=pv_restau" target="_blank" class="pos-toolk btn btn-sm btn-outline-secondary flex-fill fw-semibold ms-auto"><i class="fa fa-print"></i> Ticket</a>
                                            {{-- LIEU --}}
                                    ...
                                    <div class="d-flex flex-column mb-2"> 
                                        <div class="validation d-flex gap-1 px-1">
                                            @if($test_vide > 0)
                                                @if($this->etat == "Payée" || $this->etat == "Impayée")
                                                    <button type="button" class="pos-primary-action" wire:click="NewCommande()" title="Cliquez pour enregistrer et passer à une nouvelle commmande"><i class="fa fa-check-circle"></i> Enregistrer la commande</button>
                                                @else
                                                    @if($activer_ecran_cuisine == 1)
                                                        <button class="btn btn-lg btn-outline-dark fw-bold flex-fill py-2" wire:click="EnvoyerCuisine()" title="Cliquez pour envoyer en cuisine" data-toggle="tooltip"><i class="fa fa-refresh fa-spin"></i> Envoyer en cuisine</button>
                                                    @endif
                                                        <button class="btn btn-lg btn-outline-green fw-bold flex-fill py-2" wire:click="AffichePaie()" data-bs-toggle="modal" data-bs-target="#saissieReglementPosModal"><i class="fas fa-file-invoice-dollar"></i> Paiement</button>
                                                @endif
                                            @endif
                                        </div>
                                    </div>                                    
