...
<html lang="en">
  ...
  <body>
      ...
      <div id="app" class="app app-header-fixed app-sidebar-fixed"> 
        @include('livewire.connection_offline_online')
        @include('components.layouts.partials.menu_top')
        @include('components.layouts.partials.menu_principale')    
        {{$slot}} 
    ...
    <script>     
        document.addEventListener('livewire:init', () => {                
            Livewire.on('categorietUpdate', (event) => {
              $('#updateCategorieModal').modal('hide')
            });
            Livewire.on('deviseTvaUpdate',()=>{ 
              $('#updateDeviseTvaModal').modal('hide')
            });
            Livewire.on('produitUpdate',()=>{ 
              $('#updateProduitModal').modal('hide')
            });
            Livewire.on('imageUpdate',()=>{ 
              $('#imageProduitModal').modal('hide')
            });
            Livewire.on('correctioStock',()=>{ 
              $('#correctionStockModal').modal('hide')
            });
            Livewire.on('fermerPayer',()=>{ 
              $('#saissieReglementModal').modal('hide')
            }); 
            Livewire.on('emplacementAjouter',()=>{ 
              $('#createEmplacementModal').modal('hide')
            }); 
            Livewire.on('emplacementUpdate',()=>{ 
              $('#updateEmplacementModal').modal('hide')
            });
            Livewire.on('etapeUpdate',()=>{ 
              $('#updateEtapeModal').modal('hide')
            }); 
            Livewire.on('pipelineStore',()=>{ 
              $('#creerOppotuniteModal').modal('hide')
            }); 
            Livewire.on('grilleUpdate',()=>{ 
              $('#updateGrilleModal').modal('hide')
            });                       
